...
RegisterNetEvent('TA2Core:Client:UpdateObject', function() TA2Core = exports['ta2-core']:GetCoreObject() end)
...
local function openMenu(data, sort, skipFirst)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			if TA2Core.Shared.Items[tostring(v["icon"])] then
				if not string.find(TA2Core.Shared.Items[tostring(v["icon"])].image, "//") and not string.find(v["icon"], "//") then
                    v["icon"] = "nui://qb-inventory/html/images/"..TA2Core.Shared.Items[tostring(v["icon"])].image
				end
			end
		end
	end
...
RegisterNetEvent('ta2-menu:client:openMenu', function(data, sort, skipFirst)
    openMenu(data, sort, skipFirst)
end)

RegisterNetEvent('ta2-menu:client:closeMenu', function()
    closeMenu()
end)
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isQBCommand then
                    TriggerServerEvent('TA2Core:CallCommand', data.params.event, data.params.args)
...
RegisterNUICallback('closeMenu', function(_, cb)
    ...
    SetNuiFocus(false)
    cb('ok')
    TriggerEvent("ta2-menu:client:menuClosed")
end)
