...
local function CraftItem(craftedItem, requiredItems, amountToCraft, xpEarned, xpType)
    TA2Core.Functions.TriggerCallback('crafting:getPlayerInventory', function(inventory)
        ...
        for _, reqItem in pairs(requiredItems) do
            ...
            if itemAmount < reqItem.amount then
                ...
                TA2Core.Functions.Notify(string.format(Lang:t('notifications.notenoughMaterials')) .. amountToCraft .. 'x ' .. TA2Core.Shared.Items[craftedItem].label, 'error')
                break
            end
        end
        if hasAllMaterials then
            if Config.EnableSkillCheck then
                local success = exports['ta2-minigames']:Skillbar('easy', '12345') -- difficulty and words to enter 
                if success then
                    TA2Core.Functions.Progressbar('crafting_item', 'Crafting ' .. TA2Core.Shared.Items[craftedItem].label, (math.random(2000, 5000) * amountToCraft), false, true, {
                        ...
                        TriggerServerEvent('ta2-crafting:server:receiveItem', craftedItem, requiredItems, amountToCraft, xpEarned, xpType)
                    end)
                else
                    ...
                    TriggerServerEvent('ta2-crafting:server:removeMaterials', randomItem.item, randomAmount)
                    TA2Core.Functions.Notify('Crafting failed, some materials have been lost!', 'error')
                end
            else
                TA2Core.Functions.Progressbar('crafting_item', 'Crafting ' .. TA2Core.Shared.Items[craftedItem].label, (math.random(2000, 5000) * amountToCraft), false, true, {
                    disableMovement = true,
            ...
            TA2Core.Functions.Notify(string.format(Lang:t('notifications.notenoughMaterials')), 'error')
        end
    end)
end
...
local function CraftAmount(craftedItem, requiredItems, xpGain, xpType)
    ...
    if dialog and tonumber(dialog.amount) then
        ...
        if amount > 0 then
            ...
            TA2Core.Functions.Notify(string.format(Lang:t('notifications.invalidAmount')), 'error')
        end
    else
        TA2Core.Functions.Notify(string.format(Lang:t('notifications.invalidInput')), 'error')
    end
end
...
local function OpenCraftingMenu(benchType)
    ...
    TA2Core.Functions.TriggerCallback('crafting:getPlayerInventory', function(inventory)
        ...
        for _, recipe in pairs(recipes) do
            if currentXP >= recipe.xpRequired then
                ...
                for _, reqItem in pairs(recipe.requiredItems) do
                    ...
                    local itemLabel = TA2Core.Shared.Items[reqItem.item].label
                ...
                local menuItem = {
                    header = TA2Core.Shared.Items[recipe.item].label,
                    txt = itemsText,
                    icon = Config.ImageBasePath .. TA2Core.Shared.Items[recipe.item].image,
                    params = {
                        isAction = true,
                        event = function()
        ...
        exports['ta2-menu']:openMenu(menuItems)
    end)
end
...
local function PickupBench(benchType)
    ...
    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        TriggerServerEvent('ta2-crafting:server:addCraftingTable', benchType)
        TA2Core.Functions.Notify(string.format(Lang:t('notifications.pickupBench')), 'success')
    end
end

-- Events

RegisterNetEvent('ta2-crafting:client:useCraftingTable', function(benchType)
    local playerPed = PlayerPedId()
    ...
    TriggerServerEvent('ta2-crafting:server:removeCraftingTable', benchType)
    exports['ta2-target']:AddTargetEntity(workbench, {
        options = {
            {
                icon = 'fas fa-tools',
