...
local function DoLimbAlert()
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            TA2Core.Functions.Notify(limbDamageMsg, 'primary')
        end
    end
end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        TA2Core.Functions.Notify(Lang:t('info.bleed_alert', { bleedstate = Config.BleedingStates[tonumber(isBleeding)].label }), 'error')
    end
end
...
local function CheckWeaponDamage(ped)
    ...
    for k, v in pairs(TA2Core.Shared.Weapons) do
        if HasPedBeenDamagedByWeapon(ped, k, 0) then
            detected = true
...
RegisterNetEvent('hospital:client:ambulanceAlert', function(coords, text)
    ...
    TA2Core.Functions.Notify({ text = text, caption = street1name .. ' ' .. street2name }, 'ambulance')
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    emsNotified = false
    TA2Core.Functions.Notify(Lang:t('info.healthy'))
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent('hospital:server:RestoreWeaponDamage')
    TA2Core.Functions.Notify(Lang:t('success.wounds_healed'), 'success')
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function()
        Wait(5)
        if isRevive then
            TA2Core.Functions.Notify(Lang:t('success.being_helped'), 'success')
...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports['ta2-policejob']:IsHandcuffed() then
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount, hospitalName)
    SetTimeout(math.random(2500, 4000), function()
        ...
        if TA2Core.Functions.GetPlayerData().charinfo.gender == 1 then
            gender = Lang:t('info.mrs')
        end
        local charinfo = TA2Core.Functions.GetPlayerData().charinfo
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            exports['ta2-core']:DrawText(Lang:t('text.bed_out'))
        ...
        while listen do
            if IsControlJustPressed(0, 38) then
                ...
                if variable == 'checkin' then
                    TriggerEvent('ta2-ambulancejob:checkin')
                    listen = false
                elseif variable == 'beds' then
                    TriggerEvent('ta2-ambulancejob:beds', hospitalIndex, bedId)
                    listen = false
                end
            end
            Wait(1)
...
RegisterNetEvent('ta2-ambulancejob:checkin', function()
    ...
    for i = 1, #Config.Locations['hospital'] do
        ...
        if distance < 3 then
            if doctorCount >= Config.MinimalDoctors then
                ...
                TA2Core.Functions.Notify('Called a Doctor', 'primary')
            else
                TriggerEvent('animations:client:EmoteCommandStart', { 'notepad' })
                TA2Core.Functions.Progressbar('hospital_checkin', Lang:t('progress.checking_in'), 2000, false, true, {
                    ...
                    if bedId then
                        ...
                        TA2Core.Functions.Notify(Lang:t('error.beds_taken'), 'error')
                    end
                end)
            end
        end
...
RegisterNetEvent('ta2-ambulancejob:beds', function(hospitalIndex, bedId)
    if bedId then
...
if Config.UseTarget then
    CreateThread(function()
        for i = 1, #Config.Locations['checking'] do
            ...
            exports['ta2-target']:AddBoxZone('checking' .. i, vector3(v.x, v.y, v.z), 3.5, 2, {
                name = 'checking' .. i,
                heading = -72,
                debugPoly = false,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        ...
                        event = 'ta2-ambulancejob:checkin',
                        label = 'Check In',
                    }
                },
                distance = 1.5
        ...
        for hospitalKey = 1, #Config.Locations['hospital'] do
            for bedKey = 1, #Config.Locations['hospital'][hospitalKey]['beds'] do
                ...
                exports['ta2-target']:AddBoxZone('beds' .. bedKey, v.coords, 2.5, 2.3, {
        ...
        for i = 1, #Config.Locations['checking'] do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        exports['ta2-core']:DrawText(Lang:t('text.call_doc'), 'left')
                        CheckInControls('checkin')
                    else
                        exports['ta2-core']:DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls('checkin')
                    end
                else
                    listen = false
            ...
            for bedKey = 1, #Config.Locations['hospital'][hospitalKey]['beds'] do
                ...
                bedCombo:onPlayerInOut(function(isPointInside, _, zone)
                    if isPointInside and not isInHospitalBed then
                        exports['ta2-core']:DrawText(Lang:t('text.lie_bed'), 'left')
