...
function MenuGarage()
    ...
    local authorizedVehicles = getAuthorizedVehicles(TA2Core.Functions.GetPlayerData().job.grade.level)
...
RegisterNetEvent('TA2Core:Client:SetDuty', function(duty)
...
function Status()
    if isStatusChecking then
        ...
        exports['ta2-menu']:openMenu(statusMenu)
    end
end
...
RegisterNetEvent('hospital:client:CheckStatus', function()
    ...
    if player ~= -1 and distance < 5.0 then
        ...
        TA2Core.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
            if result then
                for k, v in pairs(result) do
                    ...
                    elseif result['WEAPONWOUNDS'] then
                        for _, v2 in pairs(result['WEAPONWOUNDS']) do
                            TriggerEvent('chat:addMessage', {
                                color = { 255, 0, 0 },
                                multiline = false,
                                args = { Lang:t('info.status'), TA2Core.Shared.Weapons[v2].damagereason }
                            })
                        end
                    elseif result['BLEED'] > 0 then
                        ...
                        TA2Core.Functions.Notify(Lang:t('success.healthy_player'), 'success')
                    end
                end
        ...
        TA2Core.Functions.Notify(Lang:t('error.no_player'), 'error')
    end
end)

RegisterNetEvent('hospital:client:RevivePlayer', function()
    local hasItem = TA2Core.Functions.HasItem('firstaid')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            TA2Core.Functions.Progressbar('hospital_revive', Lang:t('progress.revive'), 5000, false, true, {
                ...
                TA2Core.Functions.Notify(Lang:t('success.revived'), 'success')
        ...
        TA2Core.Functions.Notify(Lang:t('error.no_firstaid'), 'error')
    end
end)

RegisterNetEvent('hospital:client:TreatWounds', function()
    local hasItem = TA2Core.Functions.HasItem('bandage')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            TA2Core.Functions.Progressbar('hospital_healwounds', Lang:t('progress.healing'), 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
                ...
                StopAnimTask(PlayerPedId(), healAnimDict, 'exit', 1.0)
                TA2Core.Functions.Notify(Lang:t('success.helped_player'), 'success')
        ...
        TA2Core.Functions.Notify(Lang:t('error.no_bandage'), 'error')
    end
end)
    ...
    CreateThread(function()
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                exports['ta2-core']:KeyPressed(38)
                if variable == 'sign' then
                    TriggerEvent('EMSToggle:Duty')
                elseif variable == 'stash' then
                    TriggerServerEvent('ta2-ambulancejob:server:stash')
                elseif variable == 'storeheli' then
                    TriggerEvent('ta2-ambulancejob:storeheli')
                elseif variable == 'takeheli' then
                    TriggerEvent('ta2-ambulancejob:pullheli')
                elseif variable == 'roof' then
                    TriggerEvent('ta2-ambulancejob:elevator_main')
                elseif variable == 'main' then
                    TriggerEvent('ta2-ambulancejob:elevator_roof')
                end
            end
            Wait(1)
        end
        ...
        while CheckVehicle do
            if IsControlJustPressed(0, 38) then
                ...
                if IsPedInAnyVehicle(ped, false) then
                    TA2Core.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
                else
...
RegisterNetEvent('ta2-ambulancejob:elevator_roof', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('ta2-ambulancejob:elevator_main', function()
    local ped = PlayerPedId()
...
CreateThread(function()
    for i = 1, #Config.Locations['vehicle'] do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' and onDuty then
                exports['ta2-core']:DrawText(Lang:t('text.veh_button'), 'left')
                EMSVehicle(i)
            else
                CheckVehicle = false
    ...
    for i = 1, #Config.Locations['helicopter'] do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' and onDuty then
                exports['ta2-core']:DrawText(Lang:t('text.heli_button'), 'left')
                EMSHelicopter(i)
            else
                CheckHeli = false
                exports['ta2-core']:HideText()
...
if Config.UseTarget then
    CreateThread(function()
        for i = 1, #Config.Locations['duty'] do
            ...
            exports['ta2-target']:AddBoxZone('duty' .. i, vector3(v.x, v.y, v.z), 1.5, 1, {
                name = 'duty' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for i = 1, #Config.Locations['stash'] do
            ...
            exports['ta2-target']:AddBoxZone('stash' .. i, vector3(v.x, v.y, v.z), 1, 1, {
                name = 'stash' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = 'server',
                        event = 'ta2-ambulancejob:server:stash',
                        icon = 'fa fa-hand',
                        label = 'Open Stash',
                        job = 'ambulance'
                    }
        ...
        for i = 1, #Config.Locations['roof'] do
            ...
            exports['ta2-target']:AddBoxZone('roof' .. i, vector3(v.x, v.y, v.z), 2, 2, {
                name = 'roof' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = 'client',
                        event = 'ta2-ambulancejob:elevator_roof',
        ...
        for i = 1, #Config.Locations['main'] do
            ...
            exports['ta2-target']:AddBoxZone('main' .. i, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = 'main' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = 'client',
                        event = 'ta2-ambulancejob:elevator_main',
                        icon = 'fas fa-hand-point-up',
                        label = 'Take Elevator',
                        job = 'ambulance'
                    },
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if not onDuty then
                    exports['ta2-core']:DrawText(Lang:t('text.onduty_button'), 'left')
                    EMSControls('sign')
                else
                    exports['ta2-core']:DrawText(Lang:t('text.offduty_button'), 'left')
                    EMSControls('sign')
                end
            else
                check = false
        ...
        stashCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if onDuty then
                    exports['ta2-core']:DrawText(Lang:t('text.pstash_button'), 'left')
                    EMSControls('stash')
                end
            else
                check = false
        ...
        roofCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if onDuty then
                    exports['ta2-core']:DrawText(Lang:t('text.elevator_main'), 'left')
                    EMSControls('main')
                else
                    exports['ta2-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
            else
                check = false
                exports['ta2-core']:HideText()
        ...
        mainCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if onDuty then
                    exports['ta2-core']:DrawText(Lang:t('text.elevator_roof'), 'left')
                    EMSControls('roof')
                else
                    exports['ta2-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
