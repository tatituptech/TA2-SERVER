    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if entering ~= 0 and not isBlacklistedVehicle(entering) then
                sleep = 2000
                local plate = TA2Core.Functions.GetPlate(entering)
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                    if IsEntityDead(driver) then
                        if not isTakingKeys then
                            ...
                            TriggerServerEvent('ta2-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                            TA2Core.Functions.Progressbar('steal_keys', Lang:t('progress.takekeys'), 2500, false, false, {
                                disableMovement = false,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true
                            }, {}, {}, {}, function() -- Done
                                TriggerServerEvent('ta2-vehiclekeys:server:AcquireVehicleKeys', plate)
                                isTakingKeys = false
                            end, function()
                                isTakingKeys = false
                            end)
                    ...
                    elseif Config.LockNPCDrivingCars then
                        TriggerServerEvent('ta2-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 2)
                    else
                        TriggerServerEvent('ta2-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                        TriggerServerEvent('ta2-vehiclekeys:server:AcquireVehicleKeys', plate)
                ...
                elseif driver == 0 and entering ~= lastPickedVehicle and not HasKeys(plate) and not isTakingKeys then
                    TA2Core.Functions.TriggerCallback('ta2-vehiclekeys:server:checkPlayerOwned', function(playerOwned)
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() and TA2Core.Functions.GetPlayerData() ~= {} then
        GetKeys()
    end
end)
...
RegisterNetEvent('ta2-vehiclekeys:client:AddKeys', function(plate)
    KeysList[plate] = true
    local ped = PlayerPedId()
    if not IsPedInAnyVehicle(ped, false) then return end

    local vehicle = GetVehiclePedIsIn(ped)
    local vehicleplate = TA2Core.Functions.GetPlate(vehicle)
    if plate ~= vehicleplate then return end

    SetVehicleEngineOn(vehicle, false, false, false)
end)

RegisterNetEvent('ta2-vehiclekeys:client:RemoveKeys', function(plate)
    KeysList[plate] = nil
end)

RegisterNetEvent('ta2-vehiclekeys:client:ToggleEngine', function()
    local EngineOn = GetIsVehicleEngineRunning(GetVehiclePedIsIn(PlayerPedId()))
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
    if HasKeys(TA2Core.Functions.GetPlate(vehicle)) then
        SetVehicleEngineOn(vehicle, not EngineOn, false, true)
    end
end)

RegisterNetEvent('ta2-vehiclekeys:client:GiveKeys', function(id)
    local targetVehicle = GetVehicle()
    if not targetVehicle then return end

    local targetPlate = TA2Core.Functions.GetPlate(targetVehicle)
    if not HasKeys(targetPlate) then
        TA2Core.Functions.Notify(Lang:t('notify.ydhk'), 'error')
        return
    end
    ...
    elseif IsPedSittingInVehicle(PlayerPedId(), targetVehicle) then -- Give keys to everyone in vehicle
        ...
        for p = 1, #otherOccupants do
            TriggerServerEvent('ta2-vehiclekeys:server:GiveVehicleKeys', GetPlayerServerId(NetworkGetPlayerIndexFromPed(otherOccupants[p])), targetPlate)
        end
    else -- Give keys to closest player
        GiveKeys(GetPlayerServerId(TA2Core.Functions.GetClosestPlayer()), targetPlate)
    end
end)

RegisterNetEvent('TA2Core:Client:VehicleInfo', function(data)
    if data.event == 'Entering' then
        robKeyLoop()
    end
end)

RegisterNetEvent('ta2-weapons:client:DrawWeapon', function()
    Wait(2000)
    robKeyLoop()
end)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    if HasKeys(TA2Core.Functions.GetPlate(vehicle)) then return end
    if #(pos - GetEntityCoords(vehicle)) > 2.5 then return end
    if GetVehicleDoorLockStatus(vehicle) <= 0 then return end

    local difficulty = isAdvanced and 'easy' or 'medium' -- Easy for advanced lockpick, medium by default
    local success = exports['ta2-minigames']:Skillbar(difficulty)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == ped then
            TriggerServerEvent('ta2-vehiclekeys:server:AcquireVehicleKeys', TA2Core.Functions.GetPlate(vehicle))
        else
            TA2Core.Functions.Notify(Lang:t('notify.vlockpick'), 'success')
            TriggerServerEvent('ta2-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(vehicle), 1)
        end
    else
    ...
    if chance <= threshold then
        TriggerServerEvent('ta2-vehiclekeys:server:breakLockpick', pickType)
    end
end)
...
function ToggleEngine(veh)
    ...
    if not HasKeys(TA2Core.Functions.GetPlate(veh)) and not AreKeysJobShared(veh) then return end
...
function ToggleVehicleLocksWithoutNui(veh)
    ...
    if isBlacklistedVehicle(veh) then
        TriggerServerEvent('ta2-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
        return
    end

    if not HasKeys(TA2Core.Functions.GetPlate(veh)) and not AreKeysJobShared(veh) then
        TA2Core.Functions.Notify(Lang:t('notify.ydhk'), 'error')
        return
    end
    ...
    if vehLockStatus == 1 then
        TriggerServerEvent('ta2-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 2)
        TA2Core.Functions.Notify(Lang:t('notify.vlock'), 'primary')
    else
        TriggerServerEvent('ta2-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
        TA2Core.Functions.Notify(Lang:t('notify.vunlock'), 'success')
    end
...
function GiveKeys(id, plate)
    ...
    if distance <= 0.0 or distance >= 1.5 then
        TA2Core.Functions.Notify(Lang:t('notify.nonear'), 'error')
        return
    end

    TriggerServerEvent('ta2-vehiclekeys:server:GiveVehicleKeys', id, plate)
end

function GetKeys()
    TA2Core.Functions.TriggerCallback('ta2-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
...
function GetVehicle()
    ...
    while vehicle == 0 do
        vehicle = TA2Core.Functions.GetClosestVehicle()
...
function AreKeysJobShared(veh)
    ...
    local vehPlate = TA2Core.Functions.GetPlate(veh)
    local jobName = TA2Core.Functions.GetPlayerData().job.name
    local onDuty = TA2Core.Functions.GetPlayerData().job.onduty
            ...
            for _, vehicle in pairs(v.vehicles) do
                if string.upper(vehicle) == string.upper(vehName) then
                    if not HasKeys(vehPlate) then
                        TriggerServerEvent('ta2-vehiclekeys:server:AcquireVehicleKeys', vehPlate)
                    end
                    return true
                end
            end
...
function Hotwire(vehicle, plate)
    ...
    TA2Core.Functions.Progressbar('hotwire_vehicle', Lang:t('progress.hskeys'), hotwireTime, false, true, {
        ...
        if (math.random() <= Config.HotwireChance) then
            ...
            TA2Core.Functions.Notify(Lang:t('notify.fvlockpick'), 'error')
        end
...
function CarjackVehicle(target)
    ...
    TA2Core.Functions.Progressbar('rob_keys', Lang:t('progress.acjack'), Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
        ...
        if math.random() <= carjackChance then
            ...
            TA2Core.Functions.Notify(Lang:t('notify.cjackfail'), 'error')
            FreezeEntityPosition(vehicle, false)
            SetVehicleUndriveable(vehicle, false)
            MakePedFlee(target)
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
