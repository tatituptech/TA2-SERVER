...
local function exploitBan(id, reason)
    ...
    TriggerEvent('ta2-log:server:CreateLog', 'recyclejob', 'Player Banned', 'red',
        string.format('%s was banned by %s for %s', GetPlayerName(id), 'ta2-recyclejob', reason), true)
...
TA2Core.Functions.CreateCallback('ta2-recyclejob:server:getPriceList', function(source, cb)
    local src = source
...
local function checkStock(source, item, amount)
    ...
    if Stock[item] >= amount then
        ...
        TriggerClientEvent('TA2Core:Notify', source, Lang:t('error.out_of_stock', {item = item}), 'error')
        return false
    end
end
...
local function sellMaterials(src, item, amount)
    ...
    if Player.Functions.RemoveItem(item, amount) then
        ...
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.sold', {amount = amount, item = TA2Core.Shared.Items[item].label, price = price}), 'success')
        adjustStock(item, 'add', amount)
    else
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.nothing_to_sell'), 'error')
        return
    end
end
...
local function getItem(source, item, amount)
    ...
    if Config.LimitedMaterials then
        ...
        TriggerClientEvent('ta2-inventory:client:ItemBox', source, TA2Core.Shared.Items[item], 'add', amount)
...
RegisterNetEvent('ta2-recyclejob:server:getItem', function()
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    ...
    if luckyChance <= LuckyItemChance then 
        ...
        TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items[luckyItem], 'add', 1)
    end
end)

RegisterNetEvent('ta2-recyclejob:server:sellItem', function(item, amount)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
