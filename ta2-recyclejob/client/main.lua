...
local function toggleDuty()
    if onDuty then
        TA2Core.Functions.Notify(Lang:t('text.clock_out'), 'success')
        onDuty = false
        if packageCoords then
            SetEntityDrawOutline(props[packageCoords], false)
            packageCoords = nil
        ...
        TA2Core.Functions.Notify(Lang:t('text.clock_in'), 'success')
        onDuty = true
        GetRandomPackage()
    end
end
...
local function pickUp()
    isBusy = true
    TA2Core.Functions.Progressbar('pickup_reycle_package', Lang:t('text.picking_up_the_package'), Config.PickupActionDuration, false, true, {
...
local function handInPackage()
    DropPackage()
    TA2Core.Functions.Progressbar('deliver_reycle_package', Lang:t('text.unpacking_the_package'), Config.DeliveryActionDuration, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true
        ...
        TriggerServerEvent('ta2-recyclejob:server:getItem')
        GetRandomPackage()
    end)
end

local function sellMaterials()
    TA2Core.Functions.TriggerCallback('ta2-recyclejob:server:getPriceList', function(data)
        local menu = {}
        if data == false then TA2Core.Functions.Notify(Lang:t('error.too_far_to_sell') 'error') return end
        for k, v in pairs (data) do
            if TA2Core.Functions.HasItem(k) then
                menu[#menu+1] = {
                    header = TA2Core.Shared.Items[k].label,
                    txt = Lang:t('text.price', {price = v}),
                    icon = "nui://qb-inventory/html/images/" .. TA2Core.Shared.Items[k].name .. ".png",
                    action = function()
                        local dialog = exports['ta2-input']:ShowInput({
                            header = Lang:t('text.sell') .. ' ' ..  TA2Core.Shared.Items[k].label,
                        ...
                        TriggerServerEvent('ta2-recyclejob:server:sellItem', k, tonumber(dialog.amount))
                    end
                }
            end
        end
        if #menu == 0 then
            TA2Core.Functions.Notify(Lang:t('error.nothing_to_sell'), 'error')
            return
        end
        exports['ta2-menu']:openMenu(menu)
    end)
end
...
local function Start()
           ...
           local sellZone = BoxZone:Create(vector3(loc.x, loc.y, loc.z), 2.0, 1.5, {
            ...
            sellZone:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    inZone['sellPed'] = true
                    exports['ta2-core']:DrawText(Lang:t('text.point_sell_materials'), 'left')
                else
    ...
    for k, v in pairs (Config.PickupLocations) do
        ...
        if Config.UseTarget then 
            exports['ta2-target']:AddTargetEntity(props[k], {
                options = {
                    {
                        type = 'client',
                ...
                if isPointInside then
                    if k == packageCoords then
                       inZone['targetCrate'] = true
                        exports['ta2-core']:DrawText(Lang:t('text.point_get_package'), 'left')
                    end
                else
    ...
    if Config.UseTarget then
        exports['ta2-target']:AddBoxZone('enterLocation', vector3(Config.OutsideLocation.x, Config.OutsideLocation.y, Config.OutsideLocation.z), 4, 1.5, {
            name = 'enterLocation',
            heading = 44.0,
        ...
        exports['ta2-target']:AddBoxZone('exitLocation', vector3(Config.InsideLocation.x, Config.InsideLocation.y, Config.InsideLocation.z), 4, 1.5, {
            name = 'exitLocation',
            heading = 44.0,
        ...
        exports['ta2-target']:AddBoxZone('dutyLocation', vector3(Config.DutyLocation.x, Config.DutyLocation.y, Config.DutyLocation.z), 4, 1.5, {
            name = 'dutyLocation',
            heading = 44.0,
        ...
        exports['ta2-target']:AddBoxZone('recycleDrop', vector3(Config.DropLocation.x, Config.DropLocation.y, Config.DropLocation.z), 4, 1.5, {
            name = 'recycleDrop',
            heading = 44.0,
        ...
        enterZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                exports['ta2-core']:DrawText(Lang:t('text.point_enter_warehouse'), 'left')
        ...
        exitZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                exports['ta2-core']:DrawText(Lang:t('text.point_exit_warehouse'), 'left')
        ...
        dutyZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                exports['ta2-core']:DrawText(Lang:t('text.point_toggle_duty'), 'left')
        ...
        turnIn:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if carryPackage then
                    ...
                    exports['ta2-core']:DrawText(Lang:t('text.point_hand_in_package'), 'left')
                end
            else
                exports['ta2-core']:HideText()
                inZone['turnIn'] = isPointInside
