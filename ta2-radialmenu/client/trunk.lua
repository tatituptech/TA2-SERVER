...
RegisterNetEvent('ta2-kidnapping:client:SetKidnapping', function(bool)
    isKidnapping = bool
end)

RegisterNetEvent('ta2-trunk:client:KidnapTrunk', function()
    local closestPlayer, distance = TA2Core.Functions.GetClosestPlayer()
    if distance ~= -1 and distance < 2 then
        if isKidnapping then
            ...
            if closestVehicle ~= 0 then
                ...
                Wait(50)
                TriggerServerEvent('ta2-trunk:server:KidnapTrunk', GetPlayerServerId(closestPlayer), closestVehicle)
            end
        else
            TA2Core.Functions.Notify(Lang:t('error.not_kidnapped'), 'error')
        end
    end
end)

RegisterNetEvent('ta2-trunk:client:KidnapGetIn', function(veh)
    ...
    if Config.TrunkClasses[vehClass].allowed then
        TA2Core.Functions.TriggerCallback('ta2-trunk:server:getTrunkBusy', function(isBusy)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                TriggerServerEvent('ta2-trunk:server:setTrunkBusy', plate, true)
                                inTrunk = true
                                Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                TA2Core.Functions.Notify(Lang:t('success.entered_trunk'), 'success', 4000)
                                TrunkCam(true)
                                isKidnapped = true
                            else
                                TA2Core.Functions.Notify(Lang:t('error.trunk_closed'), 'error', 2500)
                            end
                        else
                            local vehicle = GetEntityAttachedTo(ped)
                            plate = TA2Core.Functions.GetPlate(vehicle)
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                local vehCoords = GetOffsetFromEntityInWorldCoords(vehicle, 0, -5.0, 0)
                                DetachEntity(ped, true, true)
                                ClearPedTasks(ped)
                                inTrunk = false
                                TriggerServerEvent('ta2-smallresources:trunk:server:setTrunkBusy', plate, nil)
                        ...
                        TA2Core.Functions.Notify(Lang:t('error.someone_in_trunk'), 'error', 2500)
                    end
                else
                    TA2Core.Functions.Notify(Lang:t('error.already_in_trunk'), 'error', 2500)
                end
            else
                TA2Core.Functions.Notify(Lang:t('error.cant_enter_trunk'), 'error', 2500)
            end
        end, plate)
    else
        TA2Core.Functions.Notify(Lang:t('error.cant_enter_trunk'), 'error', 2500)
...
RegisterNetEvent('ta2-trunk:client:GetIn', function()
    local ped = PlayerPedId()
    local closestVehicle = getNearestVeh()
    if closestVehicle ~= 0 then
        local vehClass = GetVehicleClass(closestVehicle)
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ClearPedTasks(ped)
                            inTrunk = false
                            TriggerServerEvent('ta2-trunk:server:setTrunkBusy', plate, false)
                            SetEntityCoords(ped, vehCoords.x, vehCoords.y, vehCoords.z)
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('ta2-radialmenu:trunk:server:Door', false, plate, 5)
                            else
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('ta2-radialmenu:trunk:server:Door', true, plate, 5)
                            else
