...
local function deliveryPay(source, shop)
    ...
    Player.Functions.AddMoney('bank', Config.DeliveryPrice, 'ta2-shops:deliveryPay')
    if math.random(100) <= 10 then exports['ta2-inventory']:AddItem(source, Config.RewardItem, 1, false, false, 'ta2-shops:deliveryPay') end
end
...
RegisterNetEvent('ta2-shops:server:RestockShopItems', function(shop)
    local src = source
    ...
    TriggerClientEvent('ta2-shops:client:SetShopItems', -1, shop, Config.Locations[shop].products)
end)

RegisterNetEvent('ta2-shops:server:UpdateShopItems', function(shop, itemData, amount) -- called from inventory
...
RegisterNetEvent('ta2-shops:server:DoBail', function(bool)
    local src = source
    ...
    if bool then
        if Player.Functions.RemoveMoney('cash', Config.TruckDeposit, 'tow-received-bail') then
            ...
            TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.paid_with_cash', { value = Config.TruckDeposit }), 'success')
            TriggerClientEvent('ta2-shops:client:SpawnVehicle', src)
        elseif Player.Functions.RemoveMoney('bank', Config.TruckDeposit, 'tow-received-bail') then
            Bail[Player.PlayerData.citizenid] = Config.TruckDeposit
            TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.paid_with_bank', { value = Config.TruckDeposit }), 'success')
            TriggerClientEvent('ta2-shops:client:SpawnVehicle', src)
        else
            TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.no_deposit', { value = Config.TruckDeposit }), 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], 'trucker-bail-paid')
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.refund_to_cash', { value = Config.TruckDeposit }), 'success')
        end
    end
end)

RegisterNetEvent('ta2-shops:server:PaySlip', function(drops)
    local src = source
...
RegisterNetEvent('ta2-shops:server:openShop', function(data)
    local src = source
    ...
    exports['ta2-inventory']:OpenShop(src, shopName)
end)
