...
function TakeOutVehicle(vehicleInfo)
    ...
    TA2Core.Functions.TriggerCallback('TA2Core:Server:SpawnVehicle', function(netId)
        ...
        TriggerServerEvent("ta2-newsjob:server:addVehicleItems", TA2Core.Functions.GetPlate(veh))
...
function MenuGarage()
    ...
    local Vehicles = Config.Vehicles[TA2Core.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "ta2-newsjob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
function MenuHeliGarage()
    ...
    local Helicopters = Config.Helicopters[TA2Core.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Helicopters) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "ta2-newsjob:client:TakeOutHelicopters",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('ta2-newsjob:client:TakeOutVehicle', function(data)
    local vehicle = data.vehicle
    TakeOutVehicle(vehicle)
end)

RegisterNetEvent('ta2-newsjob:client:TakeOutHelicopters', function(data)
    local vehicle = data.vehicle
