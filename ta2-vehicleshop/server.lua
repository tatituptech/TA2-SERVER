...
local function GetVehicleTypeByModel(model)
    local vehicleData = TA2Core.Shared.Vehicles[model]
...
TA2Core.Functions.CreateCallback('ta2-vehicleshop:server:spawnvehicle', function(source, cb, plate, vehicle, coords)
    local vehType = TA2Core.Shared.Vehicles[vehicle] and TA2Core.Shared.Vehicles[vehicle].type or GetVehicleTypeByModel(vehicle)
...
RegisterNetEvent('ta2-vehicleshop:server:addPlayer', function(citizenid)
    financetimer[citizenid] = os.time()
end)

-- Deduct stored game time from player on logout
RegisterNetEvent('ta2-vehicleshop:server:removePlayer', function(citizenid)
...
local function GeneratePlate()
    local plate = TA2Core.Shared.RandomInt(1) .. TA2Core.Shared.RandomStr(2) .. TA2Core.Shared.RandomInt(3) .. TA2Core.Shared.RandomStr(2)
...
-- Callbacks
TA2Core.Functions.CreateCallback('ta2-vehicleshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('ta2-vehicleshop:server:deleteVehicle', function(netId)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    DeleteEntity(vehicle)
end)

-- Sync vehicle for other players
RegisterNetEvent('ta2-vehicleshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('ta2-vehicleshop:client:swapVehicle', -1, data)
    Wait(1500)                                                -- let new car spawn
    TriggerClientEvent('ta2-vehicleshop:client:homeMenu', src) -- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('ta2-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    local src = source
    local target = tonumber(playerid)
    if not TA2Core.Functions.GetPlayer(target) then
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.Invalid_ID'), 'error')
        return
    end
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('ta2-vehicleshop:client:customTestDrive', target, vehicle)
    else
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.playertoofar'), 'error')
    end
end)

-- Make a finance payment
RegisterNetEvent('ta2-vehicleshop:server:financePayment', function(paymentAmount, vehData)
    local src = source
    local player = TA2Core.Functions.GetPlayer(src)
    ...
    if newBalance > 0 then
        if player and paymentAmount >= minPayment then
            ...
            elseif bank >= paymentAmount then
                ...
                TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.notenoughmoney'), 'error')
            end
        else
            TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.minimumallowed') .. comma_value(minPayment), 'error')
        end
    else
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.overpaid'), 'error')
    end
end)


-- Pay off vehice in full
RegisterNetEvent('ta2-vehicleshop:server:financePaymentFull', function(data)
    local src = source
    local player = TA2Core.Functions.GetPlayer(src)
    local cash = player.PlayerData.money['cash']
    local bank = player.PlayerData.money['bank']
    ...
    if player and vehBalance ~= 0 then
        ...
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.alreadypaid'), 'error')
    end
end)

-- Buy public vehicle outright
RegisterNetEvent('ta2-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    local vehiclePrice = TA2Core.Shared.Vehicles[vehicle]['price']
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.purchased'), 'success')
        TriggerClientEvent('ta2-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
...
RegisterNetEvent('ta2-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
    ...
    if downPayment > vehiclePrice then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.notworth'), 'error') end
    if downPayment < minDown then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.downtoosmall'), 'error') end
    if paymentAmount > Config.MaximumPayments then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.exceededmax'), 'error') end
    local plate = GeneratePlate()
...
RegisterNetEvent('ta2-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    local src = source
    local player = TA2Core.Functions.GetPlayer(src)
    local target = TA2Core.Functions.GetPlayer(tonumber(playerid))
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('ta2-vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)
            ...
            TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.earned_commission', { amount = comma_value(commission) }), 'success')
            exports['ta2-banking']:AddMoney(player.PlayerData.job.name, vehiclePrice, 'Vehicle sale')
            TriggerClientEvent('TA2Core:Notify', target.PlayerData.source, Lang:t('success.purchased'), 'success')
        elseif bank >= tonumber(vehiclePrice) then
            MySQL.insert('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, garage, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
                target.PlayerData.license,
                cid,
...
RegisterNetEvent('ta2-vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
    local src = source
    local player = TA2Core.Functions.GetPlayer(src)
    local target = TA2Core.Functions.GetPlayer(tonumber(playerid))
...
RegisterNetEvent('ta2-vehicleshop:server:checkFinance', function()
    local src = source
    local player = TA2Core.Functions.GetPlayer(src)
    ...
    if result[1] then
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('general.paymentduein', { time = Config.PaymentWarning }))
        ...
        for _, v in pairs(vehicles) do
            ...
            TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.repossessed', { plate = plate }), 'error')
        end
    end
end)

-- Transfer vehicle to player in passenger seat
TA2Core.Commands.Add('transfervehicle', Lang:t('general.command_transfervehicle'), { { name = 'ID', help = Lang:t('general.command_transfervehicle_help') }, { name = 'amount', help = Lang:t('general.command_transfervehicle_amount') } }, false, function(source, args)
    local src = source
    local buyerId = tonumber(args[1])
    local sellAmount = tonumber(args[2])
    if buyerId == 0 then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.Invalid_ID'), 'error') end
    local ped = GetPlayerPed(src)
    local targetPed = GetPlayerPed(buyerId)
    if targetPed == 0 then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.buyerinfo'), 'error') end
    local vehicle = GetVehiclePedIsIn(ped, false)
    if vehicle == 0 then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.notinveh'), 'error') end
    local plate = TA2Core.Shared.Trim(GetVehicleNumberPlateText(vehicle))
    if not plate then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.vehinfo'), 'error') end
    local player = TA2Core.Functions.GetPlayer(src)
    local target = TA2Core.Functions.GetPlayer(buyerId)
    local row = MySQL.single.await('SELECT * FROM player_vehicles WHERE plate = ?', { plate })
    if Config.PreventFinanceSelling then
        if row.balance > 0 then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.financed'), 'error') end
    end
    if row.citizenid ~= player.PlayerData.citizenid then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.notown'), 'error') end
    if #(GetEntityCoords(ped) - GetEntityCoords(targetPed)) > 5.0 then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.playertoofar'), 'error') end
    local targetcid = target.PlayerData.citizenid
    local targetlicense = TA2Core.Functions.GetIdentifier(target.PlayerData.source, 'license')
    if not target then return TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.buyerinfo'), 'error') end
    if not sellAmount then
        ...
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.gifted'), 'success')
        TriggerClientEvent('vehiclekeys:client:SetOwner', buyerId, plate)
        TriggerClientEvent('TA2Core:Notify', buyerId, Lang:t('success.received_gift'), 'success')
        return
    end
    if target.Functions.GetMoney('cash') > sellAmount then
        ...
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.soldfor') .. comma_value(sellAmount), 'success')
        TriggerClientEvent('vehiclekeys:client:SetOwner', buyerId, plate)
        TriggerClientEvent('TA2Core:Notify', buyerId, Lang:t('success.boughtfor') .. comma_value(sellAmount), 'success')
    elseif target.Functions.GetMoney('bank') > sellAmount then
        ...
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.buyertoopoor'), 'error')
    end
end)
