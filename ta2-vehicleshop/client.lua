...
AddEventHandler('TA2Core:Client:OnPlayerLoaded', function()
    PlayerData = TA2Core.Functions.GetPlayerData()
    local citizenid = PlayerData.citizenid
    TriggerServerEvent('ta2-vehicleshop:server:addPlayer', citizenid)
    TriggerServerEvent('ta2-vehicleshop:server:checkFinance')
...
RegisterNetEvent('TA2Core:Client:OnPlayerUnload', function()
    local citizenid = PlayerData.citizenid
    TriggerServerEvent('ta2-vehicleshop:server:removePlayer', citizenid)
    PlayerData = {}
end)
...
local vehHeaderMenu = {
    {
        ...
        params = {
            event = 'ta2-vehicleshop:client:showVehOptions'
        }
    }
}
...
local financeMenu = {
    {
        ...
        params = {
            event = 'ta2-vehicleshop:client:getVehicles'
        }
    }
}
...
local returnTestDrive = {
    {
        ...
        params = {
            event = 'ta2-vehicleshop:client:TestDriveReturn'
        }
    }
}
...
local function getVehName()
    return TA2Core.Shared.Vehicles[Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle]['name']
end

local function getVehPrice()
    return comma_value(TA2Core.Shared.Vehicles[Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle]['price'])
end

local function getVehBrand()
    return TA2Core.Shared.Vehicles[Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle]['brand']
end
...
local function createTestDriveReturn()
    ...
    testDriveZone:onPlayerInOut(function(isPointInside)
        if isPointInside and IsPedInAnyVehicle(PlayerPedId()) then
            ...
            exports['ta2-menu']:openMenu(returnTestDrive)
        else
        ...
        while inTestDrive do
            if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                ...
                if secondsLeft >= tonumber(1000 * testDriveTime) - 20 or GetPedInVehicleSeat(NetToVeh(testDriveVeh), -1) ~= PlayerPedId() then
                    TriggerServerEvent('ta2-vehicleshop:server:deleteVehicle', testDriveVeh)
                    testDriveVeh = 0
                    inTestDrive = false
                    SetEntityCoords(PlayerPedId(), prevCoords)
                    TA2Core.Functions.Notify(Lang:t('general.testdrive_complete'))
                end
...
local function createVehZones(shopName, entity)
    if not Config.UsingTarget then
        ...
        combo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if PlayerData and PlayerData.job and (PlayerData.job.name == Config.Shops[insideShop]['Job'] or Config.Shops[insideShop]['Job'] == 'none') then
                    exports['ta2-menu']:showHeader(vehHeaderMenu)
                end
            else
                exports['ta2-menu']:closeMenu()
            end
        ...
        exports['ta2-target']:AddTargetEntity(entity, {
            options = {
                {
                    type = 'client',
                    event = 'ta2-vehicleshop:client:showVehOptions',
                ...
                while insideShop do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'ta2-vehicleshop:client:TestDrive',
                            }
                        },
                        {
                            ...
                            params = {
                                isServer = true,
                                event = 'ta2-vehicleshop:server:buyShowroomVehicle',
                                args = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'ta2-vehicleshop:client:openFinance',
                                args = {
                                    price = getVehPrice(),
                                    buyVehicle = Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle
                                }
                        ...
                        },
                        {
                            ...
                            params = {
                                event = Config.FilterByMake and 'ta2-vehicleshop:client:vehMakes' or 'ta2-vehicleshop:client:vehCategories',
                            }
                        },
                    }
                    Wait(1000)
                ...
                while insideShop and PlayerData.job and PlayerData.job.name == Config.Shops[name]['Job'] do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'ta2-vehicleshop:client:openIdMenu',
                                args = {
                        ...
                        },
                        {
                            ...
                            icon = 'fa-solid fa-coins',
                            params = {
                                event = 'ta2-vehicleshop:client:openCustomFinance',
                                args = {
                                    price = getVehPrice(),
                                    vehicle = Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle
                                }
...
local function createFinanceZone(coords, name)
    ...
    financeZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            exports['ta2-menu']:showHeader(financeMenu)
        else
            exports['ta2-menu']:closeMenu()
        end
    end)
...
-- Events
RegisterNetEvent('ta2-vehicleshop:client:homeMenu', function()
    exports['ta2-menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent('ta2-vehicleshop:client:showVehOptions', function()
    exports['ta2-menu']:openMenu(vehicleMenu, true, true)
end)

RegisterNetEvent('ta2-vehicleshop:client:TestDrive', function()
    if not inTestDrive and ClosestVehicle ~= 0 then
        inTestDrive = true
        local prevCoords = GetEntityCoords(PlayerPedId())
        tempShop = insideShop -- temp hacky way of setting the shop because it changes after the callback has returned since you are outside the zone
        TA2Core.Functions.TriggerCallback('ta2-vehicleshop:server:spawnvehicle', function(netId, properties, vehPlate)
            local timeout = 5000
            ...
            testDriveVeh = netId
            TA2Core.Functions.Notify(Lang:t('general.testdrive_timenoti', { testdrivetime = Config.Shops[tempShop]['TestDriveTimeLimit'] }), "success")
        ...
        TA2Core.Functions.Notify(Lang:t('error.testdrive_alreadyin'), 'error')
    end
end)

RegisterNetEvent('ta2-vehicleshop:client:customTestDrive', function(data)
    if not inTestDrive then
        ...
        TA2Core.Functions.TriggerCallback('ta2-vehicleshop:server:spawnvehicle', function(netId, properties, vehPlate)
            ...
            SetVehicleEngineOn(veh, true, true, false)
            testDriveVeh = netId
            TA2Core.Functions.Notify(Lang:t('general.testdrive_timenoti', { testdrivetime = Config.Shops[tempShop]['TestDriveTimeLimit'] }))
        end, 'TESTDRIVE', Config.Shops[tempShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle, Config.Shops[tempShop]['TestDriveSpawn'], true) 
        createTestDriveReturn()
        startTestDriveTimer(Config.Shops[tempShop]['TestDriveTimeLimit'] * 60, prevCoords)
    else
...
RegisterNetEvent('ta2-vehicleshop:client:TestDriveReturn', function()
    ...
    if veh == entity then
        ...
        TA2Core.Functions.Notify(Lang:t('error.testdrive_return'), 'error')
    end
end)

RegisterNetEvent('ta2-vehicleshop:client:vehCategories', function(data)
    local catmenu = {}
    ...
    local categoryMenu = {
        {
            ...
            params = {
                event = Config.FilterByMake and 'ta2-vehicleshop:client:vehMakes' or 'ta2-vehicleshop:client:homeMenu'
            }
        }
    }
    for k, v in pairs(TA2Core.Shared.Vehicles) do
        if type(TA2Core.Shared.Vehicles[k]['shop']) == 'table' then
            for _, shop in pairs(TA2Core.Shared.Vehicles[k]['shop']) do
                if shop == insideShop and (not Config.FilterByMake or TA2Core.Shared.Vehicles[k]['brand'] == data.make) then
        ...
        elseif TA2Core.Shared.Vehicles[k]['shop'] == insideShop and (not Config.FilterByMake or TA2Core.Shared.Vehicles[k]['brand'] == data.make) then
            catmenu[v.category] = v.category
            if firstvalue == nil then
                firstvalue = v.category
            end
    ...
    if Config.HideCategorySelectForOne and tablelength(catmenu) == 1 then
        TriggerEvent('ta2-vehicleshop:client:openVehCats', { catName = firstvalue, make = Config.FilterByMake and data.make, onecat = true })
        return
    end
    for k, v in pairs(catmenu) do
        categoryMenu[#categoryMenu + 1] = {
            ...
            params = {
                event = 'ta2-vehicleshop:client:openVehCats',
                args = {
                    catName = k,
                }
            }
    ...
    exports['ta2-menu']:openMenu(categoryMenu, Config.SortAlphabetically, true)
end)

RegisterNetEvent('ta2-vehicleshop:client:openVehCats', function(data)
    local vehMenu = {
        {
            header = Lang:t('menus.goback_header'),
            icon = 'fa-solid fa-angle-left',
            params = {
                event = 'ta2-vehicleshop:client:vehCategories',
                args = {
                    make = data.make
                }
            }
    ...
    if data.onecat == true then
        vehMenu[1].params = {
            event = 'ta2-vehicleshop:client:vehMakes'
        }
    end
    for k, v in pairs(TA2Core.Shared.Vehicles) do
        if TA2Core.Shared.Vehicles[k]['category'] == data.catName then
            if type(TA2Core.Shared.Vehicles[k]['shop']) == 'table' then
                for _, shop in pairs(TA2Core.Shared.Vehicles[k]['shop']) do
                    if shop == insideShop then
                        vehMenu[#vehMenu + 1] = {
                            ...
                            params = {
                                isServer = true,
                                event = 'ta2-vehicleshop:server:swapVehicle',
                                args = {
                                    toVehicle = v.model,
            ...
            elseif TA2Core.Shared.Vehicles[k]['shop'] == insideShop then
                vehMenu[#vehMenu + 1] = {
                    header = v.name,
                    txt = Lang:t('menus.veh_price') .. v.price,
                    icon = 'fa-solid fa-car-side',
    ...
    exports['ta2-menu']:openMenu(vehMenu, Config.SortAlphabetically, true)
end)

RegisterNetEvent('ta2-vehicleshop:client:vehMakes', function()
    local makmenu = {}
    local makeMenu = {
        {
            header = Lang:t('menus.goback_header'),
            icon = 'fa-solid fa-angle-left',
            params = {
                event = 'ta2-vehicleshop:client:homeMenu'
            }
        }
    }
    for k, v in pairs(TA2Core.Shared.Vehicles) do
    ...
    exports['ta2-menu']:openMenu(makeMenu, Config.SortAlphabetically, true)
end)

RegisterNetEvent('ta2-vehicleshop:client:openFinance', function(data)
    local dialog = exports['ta2-input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('ta2-vehicleshop:server:financeVehicle', dialog.downPayment, dialog.paymentAmount, data.buyVehicle)
    end
end)

RegisterNetEvent('ta2-vehicleshop:client:openCustomFinance', function(data)
    local dialog = exports['ta2-input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('ta2-vehicleshop:server:sellfinanceVehicle', dialog.downPayment, dialog.paymentAmount, data.vehicle, dialog.playerid)
    end
end)

RegisterNetEvent('ta2-vehicleshop:client:swapVehicle', function(data)
    local shopName = data.ClosestShop
    if Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].chosenVehicle ~= data.toVehicle then
        local closestVehicle, closestDistance = TA2Core.Functions.GetClosestVehicle(vector3(Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].coords.x, Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].coords.y, Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].coords.z))
...
RegisterNetEvent('ta2-vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    tempShop = insideShop -- temp hacky way of setting the shop because it changes after the callback has returned since you are outside the zone
    TA2Core.Functions.TriggerCallback('ta2-vehicleshop:server:spawnvehicle', function(netId, properties, vehPlate)
        while not NetworkDoesNetworkIdExist(netId) do Wait(10) end
        local veh = NetworkGetEntityFromNetworkId(netId)
        Citizen.Await(CheckPlate(veh, vehPlate))
        TA2Core.Functions.SetVehicleProperties(veh, properties)
        exports['LegacyFuel']:SetFuel(veh, 100)
        TriggerEvent('vehiclekeys:client:SetOwner', vehPlate)
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleEngineOn(veh, true, true, false)
...
RegisterNetEvent('ta2-vehicleshop:client:getVehicles', function()
    TA2Core.Functions.TriggerCallback('ta2-vehicleshop:server:getVehicles', function(vehicles)
        local ownedVehicles = {}
        for _, v in pairs(vehicles) do
            local vehData = TA2Core.Shared.Vehicles[v.vehicle]
            if v.balance ~= 0 and vehData.shop == insideShop then
                ...
                ownedVehicles[#ownedVehicles + 1] = {
                    ...
                    icon = 'fa-solid fa-car-side',
                    params = {
                        event = 'ta2-vehicleshop:client:getVehicleFinance',
                        args = {
                            vehiclePlate = plate,
                            balance = v.balance,
        ...
        if #ownedVehicles > 0 then
            exports['ta2-menu']:openMenu(ownedVehicles)
        else
            TA2Core.Functions.Notify(Lang:t('error.nofinanced'), 'error', 7500)
        end
    end)
end)

RegisterNetEvent('ta2-vehicleshop:client:getVehicleFinance', function(data)
    local vehFinance = {
        {
            header = Lang:t('menus.goback_header'),
            params = {
        ...
        },
        {
            ...
            icon = 'fa-solid fa-hand-holding-dollar',
            params = {
                event = 'ta2-vehicleshop:client:financePayment',
                args = {
                    vehData = data,
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = 'ta2-vehicleshop:server:financePaymentFull',
                args = {
    ...
    exports['ta2-menu']:openMenu(vehFinance)
end)

RegisterNetEvent('ta2-vehicleshop:client:financePayment', function(data)
    local dialog = exports['ta2-input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('ta2-vehicleshop:server:financePayment', dialog.paymentAmount, data.vehData)
    end
end)

RegisterNetEvent('ta2-vehicleshop:client:openIdMenu', function(data)
    local dialog = exports['ta2-input']:ShowInput({
        header = TA2Core.Shared.Vehicles[data.vehicle]['name'],
        submitText = Lang:t('menus.submit_text'),
        inputs = {
            {
                text = Lang:t('menus.submit_ID'),
    ...
    if dialog then
        ...
        if data.type == 'testDrive' then
            TriggerServerEvent('ta2-vehicleshop:server:customTestDrive', data.vehicle, dialog.playerid)
        elseif data.type == 'sellVehicle' then
            TriggerServerEvent('ta2-vehicleshop:server:sellShowroomVehicle', data.vehicle, dialog.playerid)
        end
    end
end)
