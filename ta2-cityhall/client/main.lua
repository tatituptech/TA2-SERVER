...
local function openCityhallMenu()
    local mainMenu = {
        ...
        },
        {
            ...
            params = {
                event = 'ta2-cityhall:client:openIdentityMenu'
            }
        },
        {
            header = 'Job Center',
            txt = 'Available Jobs',
            params = {
                event = 'ta2-cityhall:client:openJobMenu'
            }
        },
        {
            header = 'Close Menu',
...
local function openIdentityMenu()
    TA2Core.Functions.TriggerCallback('ta2-cityhall:server:getIdentityData', function(licenses)
        local identityMenu = {
            {
                header = 'Identity',
                isMenuHeader = true
            },
            {
                header = '← Go Back',
                params = {
                    event = 'ta2-cityhall:client:openCityhallMenu'
                }
            }
        }
        ...
        for license, data in pairs(licenses) do
            table.insert(identityMenu, {
                ...
                txt = 'Cost: $' .. data.cost,
                params = {
                    event = 'ta2-cityhall:client:requestId',
                    args = {
                        type = license,
                        cost = data.cost
                    }
        ...
        exports['ta2-menu']:openMenu(identityMenu)
    end, closestCityhall)
end

local function openJobMenu()
    TA2Core.Functions.TriggerCallback('ta2-cityhall:server:receiveJobs', function(jobs)
        local jobMenu = {
            {
                header = 'Job Center',
                isMenuHeader = true
        ...
        for jobName, jobData in pairs(jobs) do
            table.insert(jobMenu, {
                ...
                params = {
                    event = 'ta2-cityhall:client:applyJob',
                    args = {
                        job = jobName
                    }
                }
        ...
        exports['ta2-menu']:openMenu(jobMenu)
    end)
end
        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    ...
                    action = function()
                        TriggerServerEvent('ta2-cityhall:server:sendDriverTest', Config.DrivingSchools[closestDrivingSchool].instructors)
                    end
                }
                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['ta2-core']:DrawText('[E] Take Driving Lessons')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['ta2-core']:DrawText('[E] Open Cityhall')
                            end
                        else
...
RegisterNetEvent('ta2-cityhall:client:openCityhallMenu', function()
    openCityhallMenu()
end)

RegisterNetEvent('ta2-cityhall:client:openIdentityMenu', function()
    openIdentityMenu()
end)

RegisterNetEvent('ta2-cityhall:client:openJobMenu', function()
    openJobMenu()
end)

RegisterNetEvent('ta2-cityhall:client:getIds', function()
    TriggerServerEvent('ta2-cityhall:server:getIDs')
end)

RegisterNetEvent('ta2-cityhall:client:requestId', function(data)
    if inRangeCityhall then
        ...
        if license and data.cost == license.cost then
            TriggerServerEvent('ta2-cityhall:server:requestId', data.type, closestCityhall)
            TA2Core.Functions.Notify(('You have received your %s for $%s'):format(license.label, data.cost), 'success', 3500)
        else
            TA2Core.Functions.Notify(Lang:t('error.not_in_range'), 'error')
        end
    else
        TA2Core.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
...
RegisterNetEvent('ta2-cityhall:client:applyJob', function(data)
    if inRangeCityhall then
        TriggerServerEvent('ta2-cityhall:server:ApplyJob', data.job, Config.Cityhalls[closestCityhall].coords)
    else
        TA2Core.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
end)

RegisterNetEvent('ta2-cityhall:client:sendDriverEmail', function(charinfo)
