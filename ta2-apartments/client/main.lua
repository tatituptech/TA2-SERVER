...
local function RegisterApartmentEntranceZone(apartmentID, apartmentData)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and not InApartment then
            exports['ta2-core']:DrawText(Lang:t('text.options'), 'left')
        else
...
local function RegisterApartmentEntranceTarget(apartmentID, apartmentData)
    ...
    exports['ta2-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
...
local function RegisterInApartmentZone(targetKey, coords, heading, text)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and text then
            exports['ta2-core']:DrawText(text, 'left')
        else
            exports['ta2-core']:HideText()
        end
...
local function RegisterInApartmentTarget(targetKey, coords, heading, options)
    ...
    local boxName = 'inApartmentTarget_' .. targetKey
    exports['ta2-target']:AddBoxZone(boxName, coords, 1.5, 1.5, {
        name = boxName,
        heading = heading,
        minZ = coords.z - 1.0,
        maxZ = coords.z + 5.0,
...
local function DeleteApartmentsEntranceTargets()
    if Apartments.Locations and next(Apartments.Locations) then
        for id, apartment in pairs(Apartments.Locations) do
            if UseTarget then
                exports['ta2-target']:RemoveZone('apartmentEntrance_' .. id)
            else
...
local function DeleteInApartmentTargets()
    ...
    if InApartmentTargets and next(InApartmentTargets) then
        for id, apartmentTarget in pairs(InApartmentTargets) do
            if UseTarget then
                exports['ta2-target']:RemoveZone('inApartmentTarget_' .. id)
            else
...
local function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Wait(250)
    TA2Core.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            TA2Core.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['ta2-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('ta2-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('ta2-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('ta2-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_close', 0.1)
            TriggerServerEvent('apartments:server:setCurrentApartment', CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('ta2-interior:client:SetNewState', true)
            else
                TriggerEvent('ta2-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('ta2-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_open', 0.1)
    openHouseAnim()
    TriggerServerEvent('ta2-apartments:returnBucket')
    DoScreenFadeOut(500)
    ...
    exports['ta2-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('ta2-apartments:server:SetInsideMeta', CurrentApartment, false)
...
local function SetClosestApartment()
    ...
    if current ~= ClosestHouse and LocalPlayer.state.isLoggedIn and not InApartment then
        ...
        TA2Core.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
            DeleteApartmentsEntranceTargets()
            DeleteInApartmentTargets()
        end, ClosestHouse)
...
function MenuOwners()
    TA2Core.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            TA2Core.Functions.Notify(Lang:t('error.nobody_home'), 'error', 3500)
            CloseMenuFull()
        else
            local apartmentMenu = {
                {
            ...
            exports['ta2-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    TA2Core.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('ta2-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('ta2-spawn:client:openUI', true)
            TriggerEvent('apartments:client:SetHomeBlip', result.type)
        else
            if Apartments.Starting then
                TriggerEvent('ta2-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('ta2-spawn:client:openUI', true)
            else
                TriggerEvent('ta2-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('ta2-spawn:client:openUI', true)
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if RangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            TA2Core.Functions.Notify(Lang:t('error.to_far_from_door'))
            return
        end
    end
...
RegisterNetEvent('ta2-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:RingDoor', function(player, _)
    ...
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'doorbell', 0.1)
    TA2Core.Functions.Notify(Lang:t('info.at_the_door'))
end)
