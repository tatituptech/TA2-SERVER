...
RegisterNetEvent('ta2-apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
    ...
    if bool then
        ...
        TA2Core.Functions.SetPlayerBucket(src, tonumber(routeId))
    else
        ...
        Player.Functions.SetMetaData('inside', insideMeta)
        TA2Core.Functions.SetPlayerBucket(src, 0)
    end
end)

RegisterNetEvent('ta2-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:openStash', function(CurrentApartment)
    local src = source
    exports['ta2-inventory']:OpenInventory(src, CurrentApartment)
end)
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label, firstSpawn)
    ...
    TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.receive_apart') .. ' (' .. label .. ')')
    if firstSpawn then
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.changed_apart'))
    TriggerClientEvent('apartments:client:SetHomeBlip', src, type)
end)
...
TA2Core.Functions.CreateCallback('apartments:GetAvailableApartments', function(_, cb, apartment)
    local apartments = {}
...
TA2Core.Functions.CreateCallback('apartments:GetApartmentOffset', function(_, cb, apartmentId)
    local retval = 0
...
TA2Core.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(_, cb, apartment)
...
TA2Core.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
TA2Core.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
TA2Core.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
