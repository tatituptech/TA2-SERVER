...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "ta2-tow:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
    ...
    exports['ta2-menu']:openMenu(towMenu)
end
...
local function CreateZone(type, number)
    ...
    elseif type == "towspots" then
        event = "ta2-tow:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        exports['ta2-target']:AddBoxZone(boxName, coords, size, size, {
            minZ = coords.z - 5.0,
            maxZ = coords.z + 5.0,
            name = boxName,
            heading = heading,
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('ta2-tow:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('ta2-tow:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('ta2-tow:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('ta2-tow:client:ShowMarker', true)
                else
                    TriggerEvent('ta2-tow:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
local function deliverVehicle(vehicle)
    ...
    TA2Core.Functions.Notify(Lang:t("mission.delivered_vehicle"), "success")
    TA2Core.Functions.Notify(Lang:t("mission.get_new_vehicle"))
...
RegisterNetEvent('ta2-tow:client:SpawnVehicle', function()
...
RegisterNetEvent('jobs:client:ToggleNpc', function()
    if TA2Core.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then
            TA2Core.Functions.Notify(Lang:t("error.finish_work"), "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('ta2-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then
            ...
            if NpcOn and CurrentLocation then
                if GetEntityModel(targetVehicle) ~= joaat(CurrentLocation.model) then
                    TA2Core.Functions.Notify(Lang:t("error.vehicle_not_correct"), "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        TA2Core.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                TA2Core.Functions.Notify(Lang:t("mission.goto_depot"), "primary", 5000)
                                ...
                                TriggerServerEvent('ta2-tow:server:nano', vehNetID)
                                --remove zone
                                CurrentLocation.zoneCombo:destroy()
                            end
                            TA2Core.Functions.Notify(Lang:t("mission.vehicle_towed"), "success")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                            TA2Core.Functions.Notify(Lang:t("error.failed"), "error")
                        end)
                    end
                end
            end
        else
            TA2Core.Functions.Progressbar("untowing_vehicle", Lang:t("mission.untowing_vehicle"), 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                drawDropOff = false
                TA2Core.Functions.Notify(Lang:t("mission.vehicle_takenoff"), "success")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                TA2Core.Functions.Notify(Lang:t("error.failed"), "error")
            end)
        ...
        TA2Core.Functions.Notify(Lang:t("error.not_towing_vehicle"), "error")
    end
end)

RegisterNetEvent('ta2-tow:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('ta2-tow:server:DoBail', true, vehicleInfo)
        selectedVeh = vehicleInfo
    else
        TA2Core.Functions.Notify(Lang:t("error.too_far_away"), 'error')
    end
end)

RegisterNetEvent('ta2-tow:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('ta2-tow:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('ta2-tow:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("ta2-tow:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
        TA2Core.Functions.Notify(Lang:t("error.no_work_done"), "error")
    end
end)

RegisterNetEvent('ta2-tow:client:SpawnNPCVehicle', function()
...
RegisterNetEvent('ta2-tow:client:ShowMarker', function(active)
    if PlayerJob.name == "tow" then
        showMarker = active
    end
end)
