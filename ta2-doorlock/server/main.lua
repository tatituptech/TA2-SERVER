...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and TA2Core.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
...
TA2Core.Functions.CreateCallback('ta2-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

TA2Core.Functions.CreateCallback('ta2-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('ta2-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('ta2-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('ta2-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('ta2-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not TA2Core.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(Lang:t("general.warn_no_permission_newdoor", {player = GetPlayerName(src), license = TA2Core.Functions.GetIdentifier(src, 'license'), source = src}))
		end
		return
	end
	...
	TriggerClientEvent('ta2-doorlock:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('ta2-doorlock:server:removeLockpick', function(type)
	local Player = TA2Core.Functions.GetPlayer(source)
...
TA2Core.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('ta2-doorlock:client:addNewDoor', source)
end, Config.CommandPermission)

TA2Core.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('ta2-doorlock:client:ToggleDoorDebug', source)
end, Config.CommandPermission)
