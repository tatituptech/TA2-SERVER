...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		TA2Core.Functions.TriggerCallback('ta2-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
function SetupDoors()
	local p = promise.new()
	TA2Core.Functions.TriggerCallback('ta2-doorlock:server:setupDoors', function(result)
		p:resolve(result)
	end)
...
RegisterNetEvent('TA2Core:Player:SetPlayerData', function(val)
	PlayerData = val
end)

RegisterNetEvent('ta2-doorlock:client:setState', function(serverId, doorID, state, src, enableSounds, enableAnimation)
		...
		if current.doors then
			...
			for k, v in pairs(current.doors) do
				...
				elseif not current.locked then
					...
					if TA2Core.Shared.Round(v.currentHeading, 0) == TA2Core.Shared.Round(v.objYaw or v.objHeading, 0) then
						DoorSystemSetDoorState(v.doorHash, 4, false, false)
					end
				end
			end
			...
			elseif not current.locked then
				...
				if TA2Core.Shared.Round(current.currentHeading, 0) == TA2Core.Shared.Round(current.objYaw or current.objHeading, 0) then
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    if success then
        TA2Core.Functions.Notify(Lang:t("success.lockpick_success"), 'success', 2500)
        ...
        Wait(1800)
        -- Unlock the door
        TriggerServerEvent('ta2-doorlock:server:updateState', closestDoor.id, false, false, true, false)
    else
        TA2Core.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
        if isAdvanced then
            local chanceToRemove = math.random(1,100) <= 17
            if chanceToRemove then
                TriggerServerEvent("ta2-doorlock:server:removeLockpick", "advancedlockpick")
                TriggerEvent('ta2-inventory:client:ItemBox', TA2Core.Shared.Items["advancedlockpick"], "remove")
            end
        else
            TriggerServerEvent("ta2-doorlock:server:removeLockpick", "lockpick")
            TriggerEvent('ta2-inventory:client:ItemBox', TA2Core.Shared.Items["lockpick"], "remove")
        end
    end
end)

RegisterNetEvent('ta2-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	...
	if Config.DoorList[identifier] then
		TA2Core.Functions.Notify((Lang:t("error.door_identifier_exists")):format(identifier), 'error')
		canContinue = true
		return
	end
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		if not model or model == 0 then TA2Core.Functions.Notify(Lang:t("error.door_not_found"), 'error') canContinue = true return end
		result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
		if result then TA2Core.Functions.Notify(Lang:t("error.door_registered"), 'error') canContinue = true return end
		...
		TriggerServerEvent('ta2-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then TA2Core.Functions.Notify(Lang:t("error.door_not_found"), 'error') return end
		if entity[1] == entity[2] then TA2Core.Functions.Notify(Lang:t("error.same_entity"), 'error') canContinue = true return end
		doorData.doorHash = {}
		for i = 1, 2 do
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('ta2-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('ta2-doorlock:client:newDoorAdded', function(data, id, creatorSrc)
	Config.DoorList[id] = data
	TriggerEvent('ta2-doorlock:client:setState', creatorSrc, id, data.locked, false, true, true)
end)

RegisterNetEvent('ta2-doorlock:client:ToggleDoorDebug', function()
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('ta2-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
...
RegisterCommand('remotetriggerdoor', function()
	...
	TriggerServerEvent('ta2-doorlock:server:updateState', nearestDoor.id, not nearestDoor.data.locked, NetworkGetNetworkIdFromEntity(playerPed), false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
