    ...
    if Zone then
        ...
        if vehicles then
            for i = 1, #vehicles, 1 do
                ...
                TA2Core.Functions.SetVehicleProperties(occasionVehicles[Zone][i].car, json.decode(occasionVehicles[Zone][i].mods))
                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['ta2-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = 'client',
                                event = 'ta2-vehiclesales:client:OpenContract',
                                icon = 'fas fa-car',
...
local function despawnOccasionsVehicles()
    ...
    for i = 1, #oSlot, 1 do
        ...
        if oldVehicle then
            TA2Core.Functions.DeleteVehicle(oldVehicle)
        end

        if EntityZones[i] and Config.UseTarget then
            exports['ta2-target']:RemoveZone(EntityZones[i])
        end
    end
    EntityZones = {}
end
...
local function openSellContract(bool)
    local pData = TA2Core.Functions.GetPlayerData()
    ...
    SendNUIMessage({
        ...
        plate = TA2Core.Functions.GetPlate(GetVehiclePedIsUsing(PlayerPedId()))
    })
end
...
local function sellVehicleWait(price)
    ...
    Wait(1500)
    DoScreenFadeIn(250)
    TA2Core.Functions.Notify(Lang:t('success.car_up_for_sale', { value = price }), 'success')
    PlaySound(-1, 'SELECT', 'HUD_FRONTEND_DEFAULT_SOUNDSET', 0, 0, 1)
end

local function SellData(data, model)
    TA2Core.Functions.TriggerCallback('ta2-vehiclesales:server:CheckModelName', function(DataReturning)
        local vehicleData = {}
        vehicleData.ent = GetVehiclePedIsUsing(PlayerPedId())
        vehicleData.model = DataReturning
        vehicleData.plate = model
        vehicleData.mods = TA2Core.Functions.GetVehicleProperties(vehicleData.ent)
        vehicleData.desc = data.desc
        TriggerServerEvent('ta2-occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('ta2-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('ta2-occasions:client:MainMenu')
                        --TriggerEvent('ta2-vehiclesales:client:SellVehicle')
                    else
                        TA2Core.Functions.Notify(Lang:t('error.not_in_veh'), 'error', 4500)
                    end
                end
            end
            Wait(0)
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                TA2Core.Functions.TriggerCallback('ta2-occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('sellVehicle', function(data, cb)
    local plate = TA2Core.Functions.GetPlate(GetVehiclePedIsUsing(PlayerPedId())) --Getting the plate and sending to the function
    SellData(data, plate)
    cb('ok')
end)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('ta2-occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('ta2-occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('ta2-occasions:client:BuyFinished', function(vehdata)
    ...
    TA2Core.Functions.TriggerCallback('TA2Core:Server:SpawnVehicle', function(netId)
        ...
        TA2Core.Functions.Notify(Lang:t('success.vehicle_bought'), 'success', 2500)
        TriggerEvent('vehiclekeys:client:SetOwner', vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Wait(500)
        TA2Core.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('ta2-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        TA2Core.Functions.TriggerCallback('ta2-occasions:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('ta2-occasions:server:sellVehicleBack', vehicleData)
                    TA2Core.Functions.DeleteVehicle(vehicle)
                else
                    TA2Core.Functions.Notify(Lang:t('error.finish_payments'), 'error', 3500)
                end
            else
                TA2Core.Functions.Notify(Lang:t('error.not_your_vehicle'), 'error', 3500)
            end
...
RegisterNetEvent('ta2-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    TA2Core.Functions.TriggerCallback('TA2Core:Server:SpawnVehicle', function(netId)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        TA2Core.Functions.Notify(Lang:t('info.vehicle_returned'))
        TriggerEvent('vehiclekeys:client:SetOwner', vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Wait(500)
        TA2Core.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('ta2-occasion:client:refreshVehicles', function()
    if Zone then
        TA2Core.Functions.TriggerCallback('ta2-occasions:server:getVehicles', function(vehicles)
            despawnOccasionsVehicles()
            spawnOccasionsVehicles(vehicles)
...
RegisterNetEvent('ta2-vehiclesales:client:SellVehicle', function()
    local VehiclePlate = TA2Core.Functions.GetPlate(GetVehiclePedIsIn(PlayerPedId()))
    TA2Core.Functions.TriggerCallback('ta2-occasions:server:checkVehicleOwner', function(owned, balance)
        if owned then
            if balance < 1 then
                TA2Core.Functions.TriggerCallback('ta2-occasions:server:getVehicles', function(vehicles)
                    if vehicles == nil or #vehicles < #Config.Zones[Zone].VehicleSpots then
                        ...
                        TA2Core.Functions.Notify(Lang:t('error.no_space_on_lot'), 'error', 3500)
                    end
                end)
            else
                TA2Core.Functions.Notify(Lang:t('error.finish_payments'), 'error', 3500)
...
RegisterNetEvent('ta2-vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        TA2Core.Functions.TriggerCallback('ta2-occasions:server:getSellerInformation', function(info)
            if info then
        ...
        TA2Core.Functions.Notify(Lang:t('error.not_for_sale'), 'error', 7500)
    end
end)

RegisterNetEvent('ta2-occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'ta2-vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'ta2-occasions:client:SellBackCar',
            }
        }
    }

    exports['ta2-menu']:openMenu(MainMenu)
end)
    ...
    for k, cars in pairs(Config.Zones) do
        ...
        SpawnZone[k]:onPlayerInOut(function(isPointInside)
            if isPointInside and IsPedInAnyVehicle(PlayerPedId(), false) then
                exports['ta2-core']:DrawText(Lang:t('menu.interaction'), 'left')
                TextShown = true
                Listen4Control()
            else
                listen = false
            ...
            for k2, v in pairs(Config.Zones[k].VehicleSpots) do
                ...
                VehicleZones:onPlayerInOut(function(isPointInside)
                    if isPointInside and IsCarSpawned(k2) then
                        exports['ta2-core']:DrawText(Lang:t('menu.view_contract_int'), 'left')
                        TextShown = true
                        Listen4Control(k2)
                    else
                        listen = false
