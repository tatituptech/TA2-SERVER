...
local function updateHousePlants(leftHouse)
    ...
    CreateThread(function()
        ...
        if leftHouse then
            ...
            TA2Core.Functions.TriggerCallback('ta2-weed:server:getBuildingPlants', function(plants)
        ...
        while currentHouse do
            ...
            if plantSpawned and closestPlant ~= 0 then
                ...
                if status > 0 and IsControlJustPressed(0, 38) then
                    ...
                    TA2Core.Functions.Progressbar('remove_weed_plant', Lang:t((status == 1) and 'text.harvesting_plant' or 'text.removing_the_plant'), 8000, false, true, {
                        ...
                        if status == 1 then
                            ...
                            TriggerServerEvent('ta2-weed:server:harvestPlant', currentHouse, amount, plantData['plantSort']['name'], plantData['plantStats']['plantId'])
                        else
                            TriggerServerEvent('ta2-weed:server:removeDeathPlant', currentHouse, plantData['plantStats']['plantId'])
                        end
                    end, function() -- Cancel
                        ClearPedTasks(ped)
...
RegisterNetEvent('ta2-weed:client:getHousePlants', function(house)
    TA2Core.Functions.TriggerCallback('ta2-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
        housePlants[currentHouse] = plants
        spawnHousePlants()
...
RegisterNetEvent('ta2-weed:client:leaveHouse', function()
    updateHousePlants(true)
end)

RegisterNetEvent('ta2-weed:client:refreshHousePlants', function(house)
    if currentHouse == house or not house then
        updateHousePlants(false)
    end
end)

RegisterNetEvent('ta2-weed:client:placePlant', function(type, item)
    if not currentHouse then
        TA2Core.Functions.Notify(Lang:t('error.not_safe_here'), 'error', 3500)
        return
    end
    if closestPlant ~= 0 then
        TA2Core.Functions.Notify(Lang:t('error.cant_place_here'), 'error', 3500)
        return
    end
    local ped = PlayerPedId()
    ...
    TA2Core.Functions.Progressbar('plant_weed_plant', Lang:t('text.planting'), 8000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('ta2-weed:server:placePlant', json.encode(plantData['plantCoords']), type, currentHouse)
        TriggerServerEvent('ta2-weed:server:removeSeed', item.slot, item.name)
    end, function() -- Cancel
        ClearPedTasks(ped)
        TA2Core.Functions.Notify(Lang:t('error.process_canceled'), 'error')
        LocalPlayer.state:set('inv_busy', false, true)
...
RegisterNetEvent('ta2-weed:client:foodPlant', function()
    ...
    if closestPlant ~= 0 then
        ...
        if plantData['plantStats']['food'] == 100 then
            TA2Core.Functions.Notify(Lang:t('error.not_need_nutrition'), 'error', 3500)
        else
            LocalPlayer.state:set('inv_busy', true, true)
            TA2Core.Functions.Progressbar('plant_weed_plant', Lang:t('text.feeding_plant'), math.random(4000, 8000), false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                TriggerServerEvent('ta2-weed:server:foodPlant', currentHouse, newFood, plantData['plantSort']['name'], plantData['plantStats']['plantId'])
            end, function() -- Cancel
                ClearPedTasks(ped)
                LocalPlayer.state:set('inv_busy', false, true)
                TA2Core.Functions.Notify(Lang:t('error.process_canceled'), 'error')
        ...
        TA2Core.Functions.Notify(Lang:t('error.cant_place_here'), 'error')
    end
end)
