...
CreateThread(function()
    ...
    TriggerClientEvent('ta2-garages:client:houseGarageConfig', -1, HouseGarages)
    TriggerClientEvent('ta2-houses:client:setHouseConfig', -1, Config.Houses)
end)
...
TA2Core.Commands.Add('decorate', Lang:t('info.decorate_interior'), {}, false, function(source)
    local src = source
    TriggerClientEvent('ta2-houses:client:decorate', src)
end)

TA2Core.Commands.Add('createhouse', Lang:t('info.create_house'), { { name = 'price', help = Lang:t('info.price_of_house') }, { name = 'tier', help = Lang:t('info.tier_number') } }, true, function(source, args)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    local price = tonumber(args[1])
    local tier = tonumber(args[2])
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('ta2-houses:client:createHouses', src, price, tier)
    else
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

TA2Core.Commands.Add('addgarage', Lang:t('info.add_garage'), {}, false, function(source)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('ta2-houses:client:addGarage', src)
    else
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

TA2Core.Commands.Add('ring', Lang:t('info.ring_doorbell'), {}, false, function(source)
    local src = source
    TriggerClientEvent('ta2-houses:client:RequestRing', src)
end)

-- Item

TA2Core.Functions.CreateUseableItem('police_stormram', function(source, _)
    local Player = TA2Core.Functions.GetPlayer(source)
    if (Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty) then
        TriggerClientEvent('ta2-houses:client:HomeInvasion', source)
    else
        TriggerClientEvent('TA2Core:Notify', source, Lang:t('error.emergency_services'), 'error')
    end
end)
...
RegisterNetEvent('ta2-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent('ta2-houses:client:setHouseConfig', src, Config.Houses)
end)

RegisterNetEvent('ta2-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent('ta2-houses:client:createBlip', -1, coords)
end)

RegisterNetEvent('ta2-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", '')
    ...
    TriggerClientEvent('ta2-houses:client:setHouseConfig', -1, Config.Houses)
    TriggerClientEvent('TA2Core:Notify', src, Lang:t('info.added_house', { value = label }))
    TriggerEvent('ta2-log:server:CreateLog', 'house', Lang:t('log.house_created'), 'green', Lang:t('log.house_address', { label = label, price = price, tier = tier, agent = GetPlayerName(src) }))
end)

RegisterNetEvent('ta2-houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent('ta2-garages:client:addHouseGarage', -1, house, garageInfo)
    TriggerClientEvent('TA2Core:Notify', src, Lang:t('info.added_garage', { value = garageInfo.label }))
end)

RegisterNetEvent('ta2-houses:server:viewHouse', function(house)
    local src = source
    local pData = TA2Core.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ta2-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes, pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('ta2-houses:server:openStash', function(CurrentHouse)
    local src = source
    ...
    if stashSlots and stashWeight then
        exports['ta2-inventory']:OpenInventory(src, CurrentHouse, {
...
RegisterNetEvent('ta2-houses:server:buyHouse', function(house)
    local src = source
    local pData = TA2Core.Functions.GetPlayer(src)
    ...
    if isOwned then
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.already_owned'), 'error')
        CancelEvent()
        return
    end
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('ta2-houses:client:SetClosestHouse', src)
        TriggerClientEvent('ta2-house:client:RefreshHouseTargets', src)
        pData.Functions.RemoveMoney('bank', HousePrice, 'bought-house') -- 21% Extra house costs
        exports['ta2-banking']:AddMoney('realestate', (HousePrice / 100) * math.random(18, 25), 'House purchase')
        TriggerEvent('ta2-log:server:CreateLog', 'house', Lang:t('log.house_purchased'), 'green', Lang:t('log.house_purchased_by', { house = house:upper(), price = HousePrice, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname }))
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.house_purchased'), 'success', 5000)
    else
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.not_enough_money'), 'error')
    end
end)

RegisterNetEvent('ta2-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('ta2-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('ta2-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('ta2-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('ta2-houses:server:giveKey', function(house, target)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    local pData = TA2Core.Functions.GetPlayer(target)
    ...
    if not isHouseOwner(Player.PlayerData.license, Player.PlayerData.citizenid, house) then
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.not_owner'), 'error')
        return
    end
...
RegisterNetEvent('ta2-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.remove_key_from', { firstname = citizenData.firstname, lastname = citizenData.lastname }), 'error')
    MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
end)

RegisterNetEvent('ta2-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = TA2Core.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('ta2-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('ta2-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('ta2-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('ta2-houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', { json.encode(decorations), house })
    TriggerClientEvent('ta2-houses:server:sethousedecorations', -1, house, decorations)
end)

RegisterNetEvent('ta2-houses:server:LogoutLocation', function()
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-houses:server:giveHouseKey', function(target, house)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    local tPlayer = TA2Core.Functions.GetPlayer(target)
    ...
    if housekeyholders[house] then
        for _, cid in pairs(housekeyholders[house]) do
            if cid == tPlayer.PlayerData.citizenid then
                TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.already_keys'), 'error', 3500)
                return
            end
        end
        housekeyholders[house][#housekeyholders[house] + 1] = tPlayer.PlayerData.citizenid
        MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
        TriggerClientEvent('ta2-houses:client:refreshHouse', tPlayer.PlayerData.source)

        TriggerClientEvent('TA2Core:Notify', tPlayer.PlayerData.source, Lang:t('success.recieved_key', { value = Config.Houses[house].adress }), 'success', 2500)
    else
        local sourceTarget = TA2Core.Functions.GetPlayer(src)
        housekeyholders[house] = {
...
RegisterNetEvent('ta2-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('ta2-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('ta2-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('ta2-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('ta2-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
TA2Core.Functions.CreateCallback('ta2-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = TA2Core.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money['bank']
...
TA2Core.Functions.CreateCallback('ta2-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
TA2Core.Functions.CreateCallback('ta2-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
TA2Core.Functions.CreateCallback('ta2-houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
TA2Core.Functions.CreateCallback('ta2-houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

TA2Core.Functions.CreateCallback('ta2-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
TA2Core.Functions.CreateCallback('ta2-phone:server:TransferCid', function(_, cb, NewCid, house)
...
TA2Core.Functions.CreateCallback('ta2-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
TA2Core.Functions.CreateCallback('ta2-houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', { house })
    if result[1] then
        retval = result[1]
...
TA2Core.Functions.CreateCallback('ta2-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = TA2Core.Functions.GetPlayer(src)
    if pData then
...
TA2Core.Functions.CreateCallback('ta2-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = TA2Core.Functions.GetPlayer(src)
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetPlayerHouses', function(source, cb)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    local MyHouses = {}
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetHouseKeys', function(source, cb)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    local MyKeys = {}
...
TA2Core.Functions.CreateCallback('ta2-phone:server:MeosGetPlayerHouses', function(_, cb, input)
    if input then
