...
local function EnableEditMode()
	...
	DecoMode = true
	exports['ta2-target']:AllowTargeting(false)
	LocalPlayer.state:set('inv_busy', true, true)
	TriggerEvent('ta2-anticheat:client:ToggleDecorate', true)
end
...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent('ta2-houses:server:savedecorations', ClosestHouse, ObjectList)
	end
end
...
local function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	exports['ta2-target']:AllowTargeting(true)
	LocalPlayer.state:set('inv_busy', false, true)
	TriggerEvent('ta2-anticheat:client:ToggleDecorate', false)
end
...
--			curSpeed = 1
--		end
--		TA2Core.Functions.Notify(Lang:t('info.speed') .. tostring(speeds[curSpeed]))
--	end
--
...
RegisterNetEvent('ta2-houses:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
			...
			TA2Core.Functions.Notify(Lang:t('error.no_keys'), 'error')
		end
	else
...
RegisterNUICallback('buySelectedObject', function(data, cb)
	TA2Core.Functions.TriggerCallback('ta2-houses:server:buyFurniture', function(isSuccess)
		if isSuccess then
			SetNuiFocus(false, false)
			cursorEnabled = false
			SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				TA2Core.Functions.Notify(Lang:t('error.out_range'), 'error')
			end
		end
	end
end)
