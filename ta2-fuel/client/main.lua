...
local refuelVehicle = function (veh)
    if not veh or not DoesEntityExist(veh) then return TA2Core.Functions.Notify(Lang:t('error.no_vehicle')) end

    local ped = PlayerPedId()
    ClearPedTasks(ped)
    local canLiter = GetAmmoInPedWeapon(ped, `WEAPON_PETROLCAN`)
    local vehFuel = math.floor(exports['ta2-fuel']:GetFuel(veh) or 0)

    if canLiter == 0 then return TA2Core.Functions.Notify(Lang:t('error.no_fuel_can'), 'error') end
    if vehFuel == 100 then return TA2Core.Functions.Notify(Lang:t('error.vehicle_full'), 'error') end
    ...
    TA2Core.Functions.Progressbar('fueling_vehicle', Lang:t('progress.refueling'), Config.RefillTimePerLitre * liter * 1000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function()
        TriggerServerEvent('ta2-fuel:server:setCanFuel', canLiter - liter)
        SetPedAmmo(ped, `WEAPON_PETROLCAN`, canLiter - liter)
        exports['ta2-fuel']:SetFuel(veh, vehFuel + liter)
        TA2Core.Functions.Notify(Lang:t('success.refueled'), 'success')
        ClearPedTasks(ped)
    end, function() end)
end
...
local nozzleToVehicle = function (veh)
    if getVehicleCurrentSide(veh) ~= 'left' then return TA2Core.Functions.Notify(Lang:t('error.wrong_side'), 'error') end
...
local refillVehicleFuel = function (liter)
    ...
    if TA2Core.PlayerData.money[Config.MoneyType] < liter * Config.FuelPrice then return TA2Core.Functions.Notify(Lang:t('error.no_money'), 'error') end
    if not CurrentPump then return end
    local veh, dis = TA2Core.Functions.GetClosestVehicle()
    if not veh or veh == -1 or not DoesEntityExist(veh) then return TA2Core.Functions.Notify(Lang:t('error.no_nozzle'), 'error') end
    if not Entity(veh).state['nozzleAttached'] then return TA2Core.Functions.Notify(Lang:t('error.no_nozzle'), 'error') end
    ...
    TA2Core.Functions.LookAtEntity(veh, 5000, 5.0)
    Wait(500)
    ...
    TA2Core.Functions.Progressbar('fueling_vehicle', Lang:t('progress.refueling'), Config.RefillTimePerLitre * liter * 1000, false, true, {
        ...
        local success = TA2Core.Functions.TriggerCallback('ta2-fuel:server:refillVehicle', liter)
        if not success then return TA2Core.Functions.Notify(Lang:t('error.no_money'), 'error') end
        removeObjects()
        exports['ta2-fuel']:SetFuel(veh, math.floor(exports['ta2-fuel']:GetFuel(veh) or 0) + liter)
        TA2Core.Functions.Notify(Lang:t('success.refueled'), 'success')
        ClearPedTasks(ped)
    end, function()
        removeObjects()
...
local showFuelMenu = function ()
    if not CurrentPump then return end
    local veh, dis = TA2Core.Functions.GetClosestVehicle()
    if not veh or veh == -1 then return TA2Core.Functions.Notify(Lang:t('error.no_vehicle')) end
    if dis > 5 then return TA2Core.Functions.Notify(Lang:t('error.no_vehicle')) end
    SendNUIMessage({
        action = 'show',
        price = Config.FuelPrice,
        currentFuel = math.floor(exports['ta2-fuel']:GetFuel(veh) or 0),
    })
...
local setUpTarget = function ()
    for _, hash in pairs(Config.PumpModels) do
        exports['ta2-target']:AddTargetModel(hash, {
            options = {
                {
                    num = 1,
                ...
                },
                {
                    num = 4,
                    type = 'server',
                    event = 'ta2-fuel:server:buyJerryCan',
                ...
                },
                {
                    num = 5,
                    type = 'server',
                    event = 'ta2-fuel:server:refillJerryCan',
    ...
    exports['ta2-target']:AddGlobalVehicle({
        options = {
            {
                num = 1,
                icon = 'fa-solid fa-gas-pump',
...
RegisterNetEvent('TA2Core:Player:SetPlayerData', function(pData)
    TA2Core.PlayerData = pData
end)
...
CreateThread(function()
    Wait(500) -- Wait for TA2Core to load
