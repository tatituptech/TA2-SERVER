...
local SetFuel = function (vehicle, fuel)
    ...
    if Config.SyncFuelBetweenPlayers then Entity(vehicle).state:set('ta2-fuel', fuel + 0.0, true) end
end
...
if Config.SyncFuelBetweenPlayers then
    ...
    AddStateBagChangeHandler("ta2-fuel", nil, function(bagName, _, value)
        ...
        CreateThread(function ()
            while DoesEntityExist(vehicle) and GetPedInVehicleSeat(vehicle, -1) == playerPed do
                if IsVehicleEngineOn(vehicle) then
                    Entity(vehicle).state:set('ta2-fuel', GetVehicleFuelLevel(vehicle) + 0.0, true)
                end
        ...
        CreateThread(function ()
            while DoesEntityExist(vehicle) and GetPedInVehicleSeat(vehicle, -1) == playerPed do
                if IsVehicleEngineOn(vehicle) then
                    ...
                    Entity(vehicle).state:set('ta2-fuel', fuel + 0.0, true)
                end
                Wait(Config.FuelSyncTime*1000)
            end
        end)
