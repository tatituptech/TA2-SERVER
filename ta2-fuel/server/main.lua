...
TA2Core.Functions.CreateCallback('ta2-fuel:server:refillVehicle', function (src, cb, litres)
...
RegisterServerEvent('ta2-fuel:server:buyJerryCan', function ()
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterServerEvent('ta2-fuel:server:refillJerryCan', function ()
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterServerEvent('ta2-fuel:server:setCanFuel', function (fuel)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if not Player then return end
