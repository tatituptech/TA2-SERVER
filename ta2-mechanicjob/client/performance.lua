local function GetArmor(vehicle)
    ...
    for i = -1, GetNumVehicleMods(vehicle, 16) - 1 do
        ...
        local armorItem = {
            ...
            disabled = disabled,
            params = {
                event = 'ta2-mechanicjob:client:install',
                args = {
                    upgradeType = 'armor',
                    modType = 16,
                    upgradeIndex = i,
    ...
    exports['ta2-menu']:openMenu(armorMenu, true)
end
...
local function GetBrakes(vehicle)
    ...
    exports['ta2-menu']:openMenu(brakesMenu, true)
end
...
local function GetEngine(vehicle)
    ...
    exports['ta2-menu']:openMenu(engineMenu, true)
end
...
local function GetSuspension(vehicle)
    ...
    exports['ta2-menu']:openMenu(suspensionMenu, true)
end
...
local function GetTransmission(vehicle)
    ...
    exports['ta2-menu']:openMenu(transmissionMenu, true)
end
...
local function GetTurbo(vehicle)
    ...
    exports['ta2-menu']:openMenu(turboMenu, true)
end

-- Events

RegisterNetEvent('ta2-mechanicjob:client:install', function(data)
    ...
    TA2Core.Functions.Progressbar('mechanic_install', string.format(Lang:t('progress.installing'), partName), 5000, false, true, {
        ...
        TriggerServerEvent('ta2-mechanicjob:server:removeItem', 'veh_' .. partName)
        TA2Core.Functions.Notify(string.format(Lang:t('success.installed'), partName), 'success')
    end, function()
        if shouldToggleHood then ToggleHood(data.vehicle) end
    end)
end)

RegisterNetEvent('ta2-mechanicjob:client:installPart', function(item)
