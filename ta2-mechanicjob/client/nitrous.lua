        ...
        while true do
            ...
            if isDriver then
                if IsControlJustPressed(0, 155) then
                    ...
                    TriggerServerEvent('ta2-mechanicjob:server:syncNitrousFlames', netId, true)
                    nitrousActive = true
                end
                ...
                if IsControlJustReleased(0, 155) or nitrousVehicles[plate].level <= 0 then
                    ...
                    TriggerServerEvent('ta2-mechanicjob:server:syncNitrousFlames', netId, false)
                    if nitrousVehicles[plate].level <= 0 then
                        nitrousVehicles[plate].hasnitro = false
                        TriggerServerEvent('ta2-mechanicjob:server:syncNitrous', plate, false)
                        ...
                        TriggerServerEvent('ta2-mechanicjob:server:syncNitrous', plate, true, nitrousVehicles[plate].level)
                    end
                end
            else
                plate = nil
...
AddEventHandler('gameEventTriggered', function(event)
    if event == 'CEventNetworkPlayerEnteredVehicle' then
        ...
        TA2Core.Functions.TriggerCallback('ta2-mechanicjob:server:getnitrousVehicles', function(vehs)
...
RegisterNetEvent('ta2-mechanicjob:client:syncNitrousFlames', function(net, toggle)
...
RegisterNetEvent('ta2-mechanicjob:client:installNitrous', function()
    if IsPedInAnyVehicle(PlayerPedId(), false) then return end
    local closestVehicle, distance = TA2Core.Functions.GetClosestVehicle()
    ...
    local vehiclePlate = TA2Core.Functions.GetPlate(closestVehicle)
    if not vehiclePlate then return end
    if not IsNearBone(closestVehicle, 'engine') then return end
    ToggleHood(closestVehicle)
    TA2Core.Functions.Progressbar('use_nos', Lang:t('progress.nitrous'), 5000, false, true, {
        ...
        ToggleHood(closestVehicle)
        TriggerServerEvent('ta2-mechanicjob:server:removeItem', 'nitrous')
        TriggerServerEvent('ta2-mechanicjob:server:syncNitrous', vehiclePlate, true, 100)
