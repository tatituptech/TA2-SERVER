...
local function RepairPart(vehicle, plate, component)
    ...
    local hasItems = exports['ta2-inventory']:HasItem(materials)
    if not hasItems then
        TA2Core.Functions.Notify(Lang:t('warning.no_materials'), 'error')
        return
    end
    local componentLabel = Config.WearableParts[component].label or component
    ToggleHood(vehicle)
    TA2Core.Functions.Progressbar('repairing_vehicle', string.format(Lang:t('progress.repairing'), componentLabel), 10000, false, true, {
        ...
        ToggleHood(vehicle)
        TA2Core.Functions.Notify(string.format(Lang:t('success.part_repaired'), componentLabel), 'success')
        for item, amount in pairs(materials) do
            TriggerServerEvent('ta2-mechanicjob:server:removeItem', item, amount)
        end
        TriggerServerEvent('ta2-mechanicjob:server:repairVehicleComponent', plate, component)
    end, function()
        ToggleHood(vehicle)
    end)
end
...
RegisterNetEvent('ta2-mechanicjob:client:PartsMenu', function()
    ...
    TA2Core.Functions.TriggerCallback('ta2-mechanicjob:server:getVehicleStatus', function(status)
...
RegisterNetEvent('ta2-mechanicjob:client:repairVehicle', function()
    local vehicle, distance = TA2Core.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    if not IsNearBone(vehicle, 'engine') then return end
    local engineHealth = GetVehicleEngineHealth(vehicle)
    local bodyHealth = GetVehicleBodyHealth(vehicle)
    ToggleHood(vehicle)
    TA2Core.Functions.Progressbar('repairing_vehicle', Lang:t('progress.repair_vehicle'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ToggleHood(vehicle)
        TA2Core.Functions.Notify(Lang:t('success.repaired'), 'success')
        TriggerServerEvent('ta2-mechanicjob:server:removeItem', 'repairkit')
    end, function()
        ToggleHood(vehicle)
    end)
end)

RegisterNetEvent('ta2-mechanicjob:client:repairVehicleFull', function()
    local vehicle, distance = TA2Core.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    if not IsNearBone(vehicle, 'engine') then return end
    ToggleHood(vehicle)
    TA2Core.Functions.Progressbar('repairing_vehicle', Lang:t('progress.repair_vehicle'), 10000, false, true, {
        ...
        ToggleHood(vehicle)
        TA2Core.Functions.Notify(Lang:t('success.repaired'), 'success')
        TriggerServerEvent('ta2-mechanicjob:server:removeItem', 'advancedrepairkit')
    end, function()
        ToggleHood(vehicle)
    end)
end)

RegisterNetEvent('ta2-mechanicjob:client:repairTire', function()
    local vehicle, distance = TA2Core.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    local closestWheel = GetClosestWheel(vehicle)
    if not closestWheel then return end
    TA2Core.Functions.Progressbar('repairing_tire', Lang:t('progress.repair_tire'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        TA2Core.Functions.Notify(Lang:t('success.tire_repaired'), 'success')
        TriggerServerEvent('ta2-mechanicjob:server:removeItem', 'tirerepairkit')
    end)
end)

RegisterNetEvent('ta2-mechanicjob:client:cleanVehicle', function()
    local vehicle, distance = TA2Core.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    TA2Core.Functions.Progressbar('cleaning_vehicle', Lang:t('progress.cleaning'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        TA2Core.Functions.Notify(Lang:t('success.cleaned'), 'success')
        TriggerServerEvent('ta2-mechanicjob:server:removeItem', 'cleaningkit')
    end)
end)

RegisterNetEvent('ta2-mechanicjob:client:fixEverything', function()
    TA2Core.Functions.TriggerCallback('ta2-mechanicjob:server:hasPermission', function(hasPermission)
