...
local function PaintList(category)
    ...
    if dialog.primarypaint and dialog.secondarypaint and dialog.pearlescentpaint and dialog.wheelpaint then
        ...
        TriggerServerEvent('ta2-mechanicjob:server:sprayVehicle', netId, dialog.primarypaint, dialog.secondarypaint, dialog.pearlescentpaint, dialog.wheelpaint, colors)
    end
end
...
local function CustomColor()
    ...
    if (dialog.hex or dialog.colorpicker) and dialog.section then
        ...
        local netId = NetworkGetNetworkIdFromEntity(vehicle)
        if isPainting then return end
        TriggerServerEvent('ta2-mechanicjob:server:sprayVehicleCustom', netId, dialog.section, dialog.paintType, HexToRGB(color))
    end
end
...
function PaintCategories()
    ...
    exports['ta2-menu']:openMenu(Paints)
end
...
local function OpenInteriors(vehicle)
    ...
    exports['ta2-menu']:openMenu(mods)
end
...
local function WindowTint(vehicle)
    ...
    exports['ta2-menu']:openMenu(tints)
end
...
local function PlateIndex(vehicle)
    ...
    exports['ta2-menu']:openMenu(plates)
end

-- Events

RegisterNetEvent('ta2-mechanicjob:client:vehicleSetColors', function(netId, section, colorIndex)
    ...
    local props = TA2Core.Functions.GetVehicleProperties(vehicle)
    TriggerServerEvent('ta2-mechanicjob:server:SaveVehicleProps', props)
end)

RegisterNetEvent('ta2-mechanicjob:client:startParticles', function(netId, color)
    isPainting = true
    local vehicle = NetworkGetEntityFromNetworkId(netId)
...
RegisterNetEvent('ta2-mechanicjob:client:stopParticles', function()
    isPainting = false
...
RegisterNetEvent('ta2-mechanicjob:client:installCosmetic', function(item)
    local vehicle, distance = TA2Core.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    ...
    if item == 'veh_interior' then
        OpenInteriors(vehicle)
        TriggerServerEvent('ta2-mechanicjob:server:removeItem', item)
