...
local function GetDeliveryLocation()
    ...
    PolyZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inRange = true
            exports["ta2-core"]:DrawText(Lang:t('info.busstop_text'), 'rgb(220, 20, 60)')
            CreateThread(function()
                repeat
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        TA2Core.Functions.Notify(Lang:t('success.dropped_off'), 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
                        ...
                        resetNpcTask()
                        nextStop()
                        TriggerEvent('ta2-busjob:client:DoBusNpc')
...
local function busGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = "ta2-busjob:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("ta2-busjob:client:TakeVehicle", function(data)
    local coords = Config.Location
    if (BusData.Active) then
        TA2Core.Functions.Notify(Lang:t('error.one_bus_active'), 'error')
        return
    else
...
RegisterNetEvent('ta2-busjob:client:DoBusNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            TA2Core.Functions.Notify(Lang:t('info.goto_busstop'), 'primary')
                    ...
                    CreateThread(function()
                        repeat
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('ta2-busjob:server:NpcPay')
                                exports["ta2-core"]:HideText()
                                PolyZone:destroy()
                                break
                            end
            ...
            TA2Core.Functions.Notify(Lang:t('error.already_driving_bus'), 'error')
        end
    else
        TA2Core.Functions.Notify(Lang:t('error.not_in_bus'), 'error')
    end
end)
            ...
            if isPointInside then
                ...
                CreateThread(function()
                    repeat
                        Wait(5)
                        if not inVeh then
                            exports["ta2-core"]:DrawText(Lang:t('info.busstop_text'), 'left')
                            ...
                            exports["ta2-core"]:DrawText(Lang:t('info.bus_stop_work'), 'left')
                            if IsControlJustReleased(0, 38) then
                                if (not NpcData.Active or NpcData.Active and NpcData.NpcTaken == false) then
                                    ...
                                    TA2Core.Functions.Notify(Lang:t('error.drop_off_passengers'), 'error')
                                end
                            end
                        end
                    until not inRange
