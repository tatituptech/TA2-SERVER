...
local function filterVehiclesByCategory(vehicles, category)
    ...
    for _, vehicle in pairs(vehicles) do
        local vehicleData = TA2Core.Shared.Vehicles[vehicle.vehicle]
...
TA2Core.Functions.CreateCallback('ta2-garages:server:getHouseGarage', function(_, cb, house)
    local houseInfo = MySQL.single.await('SELECT * FROM houselocations WHERE name = ?', { house })
    cb(houseInfo)
end)

TA2Core.Functions.CreateCallback('ta2-garages:server:GetGarageVehicles', function(source, cb, garage, type, category)
...
TA2Core.Functions.CreateCallback('ta2-garages:server:spawnvehicle', function(source, cb, plate, vehicle, coords)
...
TA2Core.Functions.CreateCallback('ta2-garages:server:IsSpawnOk', function(_, cb, plate, type)
...
TA2Core.Functions.CreateCallback('ta2-garages:server:canDeposit', function(source, cb, plate, type, garage, state)
    local Player = TA2Core.Functions.GetPlayer(source)
    ...
    if type == 'house' and not exports['ta2-houses']:hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, Config.Garages[garage].houseName) then
        cb(false)
        return
    end
    if state == 1 then
...
RegisterNetEvent('ta2-garages:server:updateVehicleStats', function(plate, fuel, engine, body)
    local src = source
...
RegisterNetEvent('ta2-garages:server:updateVehicleState', function(state, plate)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('ta2-garages:server:UpdateOutsideVehicle', function(plate, vehicleNetID)
...
RegisterNetEvent('ta2-garages:server:trackVehicle', function(plate)
    local src = source
    local vehicleData = OutsideVehicles[plate]
    if vehicleData and DoesEntityExist(vehicleData.entity) then
        TriggerClientEvent('ta2-garages:client:trackVehicle', src, GetEntityCoords(vehicleData.entity))
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('success.vehicle_tracked'), 'success')
    else
        TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.vehicle_not_tracked'), 'error')
    end
end)

RegisterNetEvent('ta2-garages:server:PayDepotPrice', function(data)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    ...
    MySQL.scalar('SELECT depotprice FROM player_vehicles WHERE plate = ?', { data.plate }, function(result)
        if result then
            ...
            if cashBalance >= depotPrice then
                ...
                TriggerClientEvent('ta2-garages:client:takeOutGarage', src, data)
            elseif bankBalance >= depotPrice then
                Player.Functions.RemoveMoney('bank', depotPrice, 'paid-depot')
                TriggerClientEvent('ta2-garages:client:takeOutGarage', src, data)
            else
                TriggerClientEvent('TA2Core:Notify', src, Lang:t('error.not_enough'), 'error')
            end
        end
    end)
end)
...
RegisterNetEvent('ta2-garages:server:syncGarage', function(updatedGarages)
    Config.Garages = updatedGarages
end)

--Call from qb-phone

TA2Core.Functions.CreateCallback('ta2-garages:server:GetPlayerVehicles', function(source, cb)
    local Player = TA2Core.Functions.GetPlayer(source)
    local Vehicles = {}
    ...
    MySQL.rawExecute('SELECT * FROM player_vehicles WHERE citizenid = ?', { Player.PlayerData.citizenid }, function(result)
        if result[1] then
            for _, v in pairs(result) do
                local VehicleData = TA2Core.Shared.Vehicles[v.vehicle]
