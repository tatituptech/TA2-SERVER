...
local function OpenGarageMenu(data)
    TA2Core.Functions.TriggerCallback('ta2-garages:server:GetGarageVehicles', function(result)
        if result == nil then return TA2Core.Functions.Notify(Lang:t('error.no_vehicles'), 'error', 5000) end
        local formattedVehicles = {}
        for _, v in pairs(result) do
            local enginePercent = TA2Core.Shared.Round(v.engine, 0)
            local bodyPercent = TA2Core.Shared.Round(v.body, 0)
            local vname = nil
            pcall(function()
                vname = TA2Core.Shared.Vehicles[v.vehicle].name
            end)
...
local function DepositVehicle(veh, data)
    local plate = TA2Core.Functions.GetPlate(veh)
    TA2Core.Functions.TriggerCallback('ta2-garages:server:canDeposit', function(canDeposit)
        if canDeposit then
            local bodyDamage = math.ceil(GetVehicleBodyHealth(veh))
            local engineDamage = math.ceil(GetVehicleEngineHealth(veh))
            local totalFuel = exports[Config.FuelResource]:GetFuel(veh)
            TriggerServerEvent('ta2-mechanicjob:server:SaveVehicleProps', TA2Core.Functions.GetVehicleProperties(veh))
            TriggerServerEvent('ta2-garages:server:updateVehicleStats', plate, totalFuel, engineDamage, bodyDamage)
            CheckPlayers(veh)
            if plate then TriggerServerEvent('ta2-garages:server:UpdateOutsideVehicle', plate, nil) end
            TA2Core.Functions.Notify(Lang:t('success.vehicle_parked'), 'primary', 4500)
        else
            TA2Core.Functions.Notify(Lang:t('error.not_owned'), 'error', 3500)
        end
...
local function CreateBlipsZones()
    ...
    comboZone:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if GetVehiclePedIsUsing(PlayerPedId()) ~= 0 then
                            ...
                            if not IsVehicleAllowed(zone.data.category, currentVehicle) then
                                TA2Core.Functions.Notify(Lang:t('error.not_correct_type'), 'error', 3500)
                                return
                            end
            ...
            exports['ta2-core']:DrawText(displayText, 'left')
        else
            listenForKey = false
...
function GetSpawnPoint(garage)
    ...
    if not location then
        TA2Core.Functions.Notify(Lang:t('error.vehicle_occupied'), 'error')
    end
    return location
end
...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    TriggerEvent('ta2-garages:client:takeOutGarage', data)
    cb('ok')
end)

RegisterNUICallback('trackVehicle', function(plate, cb)
    TriggerServerEvent('ta2-garages:server:trackVehicle', plate)
    cb('ok')
end)
...
RegisterNUICallback('takeOutDepo', function(data, cb)
    ...
    if depotPrice ~= 0 then
        TriggerServerEvent('ta2-garages:server:PayDepotPrice', data)
    else
        TriggerEvent('ta2-garages:client:takeOutGarage', data)
    end
    cb('ok')
...
RegisterNetEvent('ta2-garages:client:trackVehicle', function(coords)
...
RegisterNetEvent('ta2-garages:client:takeOutGarage', function(data)
    TA2Core.Functions.TriggerCallback('ta2-garages:server:IsSpawnOk', function(spawn)
        if spawn then
            local location = GetSpawnPoint(data.garage)
            if not location then return end
            TA2Core.Functions.TriggerCallback('ta2-garages:server:spawnvehicle', function(netId, properties, vehPlate)
                ...
                TriggerServerEvent('ta2-garages:server:updateVehicleState', 0, vehPlate)
            ...
            TA2Core.Functions.Notify(Lang:t('error.not_depot'), 'error', 5000)
        end
...
local function CreateHouseZone(index, garage, zoneType)
    ...
    houseComboZones:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            exports['ta2-core']:DrawText(Lang:t('info.house_garage'), 'left')
        else
...
RegisterNetEvent('ta2-garages:client:setHouseGarage', function(house, hasKey) -- event sent periodically from housing
    ...
    if Config.Garages[formattedHouseName] then
        ...
        TA2Core.Functions.TriggerCallback('ta2-garages:server:getHouseGarage', function(garageInfo) -- create garage if not exist
            ...
            TriggerServerEvent('ta2-garages:server:syncGarage', Config.Garages)
        end, house)
    end
end)

RegisterNetEvent('ta2-garages:client:houseGarageConfig', function(houseGarages)
...
RegisterNetEvent('ta2-garages:client:addHouseGarage', function(house, garageInfo) -- event from housing on garage creation
    local formattedHouseName = string.gsub(string.lower(house), ' ', '')
    Config.Garages[formattedHouseName] = {
        houseName = house,
...
RegisterNetEvent('TA2Core:Client:OnGangUpdate', function(gang)
    PlayerGang = gang
end)

RegisterNetEvent('TA2Core:Client:OnJobUpdate', function(job)
    PlayerJob = job
end)
