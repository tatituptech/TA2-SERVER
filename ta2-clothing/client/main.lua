...
local function SaveSkin()
    ...
    TriggerServerEvent("ta2-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    local gender = TA2Core.Functions.GetPlayerData().charinfo.gender
    ...
    TriggerServerEvent("ta2-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("ta2-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    local gender = "male"
    if TA2Core.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    TA2Core.Functions.TriggerCallback('ta2-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('ta2-clothing:client:openMenu', function()
...
RegisterNetEvent('ta2-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('ta2-clothes:client:CreateFirstCharacter', function()
    TA2Core.Functions.GetPlayerData(function(pData)
...
RegisterNetEvent("ta2-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('ta2-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('ta2-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('ta2-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if TA2Core.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        TA2Core.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
RegisterNetEvent("ta2-clothing:client:adjustfacewear", function(type)
    if TA2Core.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
RegisterNetEvent('TA2Core:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ta2-clothes:loadPlayerSkin")
    PlayerData = TA2Core.Functions.GetPlayerData()
    loadStores()
--    TA2Core.Shared.Jobs = exports['ta2-jobs']:AddJobs()
end)
...
RegisterNetEvent('ta2-clothing:client:openOutfitMenu', function()
    TA2Core.Functions.TriggerCallback('ta2-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('ta2-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('TrackerError', function(_, cb)
    TA2Core.Functions.Notify(Lang:t("notify.error_bracelet"), "error")
    cb('ok')
end)
RegisterNUICallback('saveOutfit', function(data, cb)
    local ped = PlayerPedId()
    local model = GetEntityModel(ped)
    TriggerServerEvent('ta2-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('ta2-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('ta2-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    TA2Core.Functions.Notify(Lang:t('notify.info_deleteOutfit', {outfit = data.outfitName}))
    cb('ok')
end)
    ...
    if Config.UseTarget then
        CreateThread(function()
            for k, v in pairs(Config.Stores) do
                ...
                exports['ta2-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                    name = v.shopType .. k,
                    debugPoly = false,
                    minZ = v.coords.z-1,
                    maxZ = v.coords.z+1,
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not TA2Core.Shared.QBJobsStatus then
                    action = function()
                        customCamLocation = v.cameraLocation
                ...
                exports['ta2-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
	    ...
	    for k, v in pairs(Config.OutfitChangers) do
                exports['ta2-target']:AddBoxZone('OutfitChangers_'..k, v.coords, v.length, v.width, {
                    ...
                    options = {
                        {
                            type = "client",
                            event = "ta2-clothing:client:openOutfitMenu",
                            icon = "fas fa-sign-in-alt",
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        exports['ta2-core']:DrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        exports['ta2-core']:DrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        exports['ta2-core']:DrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then
                        exports['ta2-core']:DrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    ...
                    if PlayerData.gang and PlayerData.gang.name or (not TA2Core.Shared.QBJobsStatus and PlayerData.job.name) then
                        ...
                        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                            if isPointInside then
                                local zoneID = tonumber(TA2Core.Shared.SplitStr(zone.name, "_")[2])
                                local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not TA2Core.Shared.QBJobsStatus and PlayerData.job.name)
                ...
                if inZone then
                    ...
                    elseif not TA2Core.Shared.QBJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not TA2Core.Shared.QBJobsStatus and PlayerData.job.grade.level)
