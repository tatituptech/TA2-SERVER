...
local function CheckStationHits()
    ...
    if table.type(bankHits) == 'array' then TriggerClientEvent('ta2-bankrobbery:client:BankSecurity', -1, bankHits, false) end
end
...
RegisterNetEvent('ta2-bankrobbery:server:setBankState', function(bankId)
    ...
    if bankId == 'paleto' then
        if Config.BigBanks['paleto']['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['paleto']['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:setBankState', extraInfo = ' (paleto) ', source = source }))
        end
        Config.BigBanks['paleto']['isOpened'] = true
        TriggerEvent('ta2-bankrobbery:server:setTimeout')
    elseif bankId == 'pacific' then
        if Config.BigBanks['pacific']['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['pacific']['coords'][2]) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:setBankState', extraInfo = ' (pacific) ', source = source }))
        end
        ...
        if Config.SmallBanks[bankId]['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:setBankState', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        Config.SmallBanks[bankId]['isOpened'] = true
        TriggerEvent('ta2-bankrobbery:server:SetSmallBankTimeout', bankId)
    end
    TriggerClientEvent('ta2-bankrobbery:client:setBankState', -1, bankId)
    robberyBusy = true
    Config.OnRobberyStart(bankId)
end)

RegisterNetEvent('ta2-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    ...
    if #(GetEntityCoords(GetPlayerPed(source)) - Config[bankType][bankId]['lockers'][lockerId]['coords']) > 2.5 then
        return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:setLockerState', extraInfo = extraInfo, source = source }))
    end
    Config[bankType][bankId]['lockers'][lockerId][state] = bool
    TriggerClientEvent('ta2-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterNetEvent('ta2-bankrobbery:server:recieveItem', function(type, bankId, lockerId)
    local src = source
    ...
    if type == 'small' then
        if #(GetEntityCoords(GetPlayerPed(src)) - Config.SmallBanks[bankId]['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:receiveItem', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == 'item' then
                    ...
                    exports['ta2-inventory']:AddItem(src, item.item, itemAmount, false, false, 'ta2-bankrobbery:server:recieveItem')
                    TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items[item.item], 'add')
                elseif Config.RewardTypes[itemType].type == 'money' then
                    local info = {
                        worth = math.random(2300, 3200)
                    }
                    exports['ta2-inventory']:AddItem(src, 'markedbills', math.random(2, 3), false, info, 'ta2-bankrobbery:server:recieveItem')
                    TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['markedbills'], 'add')
                end
            else
                exports['ta2-inventory']:AddItem(src, 'security_card_01', 1, false, false, 'ta2-bankrobbery:server:recieveItem')
                TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['security_card_01'], 'add')
            end
        else
            exports['ta2-inventory']:AddItem(src, 'weapon_stungun', 1, false, false, 'ta2-bankrobbery:server:recieveItem')
            TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['weapon_stungun'], 'add')
        end
    elseif type == 'paleto' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['paleto']['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:receiveItem', extraInfo = ' (paleto) ', source = source }))
        end
        local itemType = math.random(#Config.RewardTypes)
        local tierChance = math.random(1, 100)
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                elseif Config.RewardTypes[itemType].type == 'money' then
                    ...
                    exports['ta2-inventory']:AddItem(src, 'markedbills', math.random(1, 4), false, info, 'ta2-bankrobbery:server:recieveItem')
                    TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['markedbills'], 'add')
                end
            else
                exports['ta2-inventory']:AddItem(src, 'security_card_02', 1, false, false, 'ta2-bankrobbery:server:recieveItem')
                TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['security_card_02'], 'add')
            end
        else
            exports['ta2-inventory']:AddItem(src, 'weapon_vintagepistol', 1, false, false, 'ta2-bankrobbery:server:recieveItem')
            TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['weapon_vintagepistol'], 'add')
        end
    elseif type == 'pacific' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['pacific']['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:receiveItem', extraInfo = ' (pacific) ', source = source }))
        end
        local itemType = math.random(#Config.RewardTypes)
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                exports['ta2-inventory']:AddItem(src, 'cryptostick', 1, false, info, 'ta2-bankrobbery:server:recieveItem')
            ...
            if chance == odd then
                exports['ta2-inventory']:AddItem(src, 'weapon_microsmg', 1, false, false, 'ta2-bankrobbery:server:recieveItem')
                TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['weapon_microsmg'], 'add')
            else
                exports['ta2-inventory']:AddItem(src, 'weapon_minismg', 1, false, false, 'ta2-bankrobbery:server:recieveItem')
                TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['weapon_minismg'], 'add')
            end
        end
    end
end)

AddEventHandler('ta2-bankrobbery:server:setTimeout', function()
    ...
    CreateThread(function()
        SetTimeout(60000 * 90, function()
            ...
            TriggerClientEvent('ta2-bankrobbery:client:ClearTimeoutDoors', -1)
...
AddEventHandler('ta2-bankrobbery:server:SetSmallBankTimeout', function(bankId)
    if robberyBusy or timeOut then return end
    timeOut = true
    CreateThread(function()
        SetTimeout(60000 * 30, function()
            ...
            TriggerClientEvent('ta2-bankrobbery:client:ResetFleecaLockers', -1, bankId)
            timeOut = false
            robberyBusy = false
...
RegisterNetEvent('ta2-bankrobbery:server:callCops', function(type, bank, coords)
    if type == 'small' then
        if not Config.SmallBanks[bank]['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:callCops', extraInfo = ' (smallbank ' .. bank .. ') ', source = source }))
        end
    elseif type == 'paleto' then
        if not Config.BigBanks['paleto']['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:callCops', extraInfo = ' (paleto) ', source = source }))
        end
    elseif type == 'pacific' then
        if not Config.BigBanks['pacific']['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'ta2-bankrobbery:server:callCops', extraInfo = ' (pacific) ', source = source }))
        end
    end
    TriggerClientEvent('ta2-bankrobbery:client:robberyCall', -1, type, coords)
end)

RegisterNetEvent('ta2-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent('ta2-bankrobbery:client:SetStationStatus', -1, key, isHit)
    if AllStationsHit() then
        exports['ta2-weathersync']:setBlackout(true)
        TriggerClientEvent('ta2-bankrobbery:client:disableAllBankSecurity', -1)
        Config.OnBlackout(true)
        CreateThread(function()
            SetTimeout(60000 * Config.BlackoutTimer, function()
                exports['ta2-weathersync']:setBlackout(false)
                TriggerClientEvent('ta2-bankrobbery:client:enableAllBankSecurity', -1)
...
RegisterNetEvent('ta2-bankrobbery:server:removeElectronicKit', function()
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if not Player then return end
    exports['ta2-inventory']:RemoveItem(src, 'electronickit', 1, false, 'ta2-bankrobbery:server:removeElectronicKit')
    TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['electronickit'], 'remove')
    exports['ta2-inventory']:RemoveItem(src, 'trojan_usb', 1, false, 'ta2-bankrobbery:server:removeElectronicKit')
    TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['trojan_usb'], 'remove')
end)

RegisterNetEvent('ta2-bankrobbery:server:removeBankCard', function(number)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if not Player then return end
    exports['ta2-inventory']:RemoveItem(src, 'security_card_' .. number, 1, false, 'ta2-bankrobbery:server:removeBankCard')
    TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['security_card_' .. number], 'remove')
end)
...
TA2Core.Functions.CreateCallback('ta2-bankrobbery:server:isRobberyActive', function(_, cb)
    cb(robberyBusy)
end)

TA2Core.Functions.CreateCallback('ta2-bankrobbery:server:GetConfig', function(_, cb)
    cb(Config.PowerStations, Config.BigBanks, Config.SmallBanks)
end)

TA2Core.Functions.CreateCallback('thermite:server:check', function(source, cb)
    local Player = TA2Core.Functions.GetPlayer(source)
    if not Player then return cb(false) end
    if exports['ta2-inventory']:RemoveItem(source, 'thermite', 1, false, 'thermite:server:check') then
        TriggerClientEvent('ta2-inventory:client:ItemBox', source, TA2Core.Shared.Items['thermite'], 'remove')
        cb(true)
    else
        cb(false)
    end
...
TA2Core.Functions.CreateUseableItem('thermite', function(source)
    local Player = TA2Core.Functions.GetPlayer(source)
    ...
    if Player.Functions.GetItemByName('lighter') then
        ...
        TriggerClientEvent('TA2Core:Notify', source, Lang:t('error.missing_ignition_source'), 'error')
    end
end)

TA2Core.Functions.CreateUseableItem('security_card_01', function(source)
    local Player = TA2Core.Functions.GetPlayer(source)
    if not Player or not Player.Functions.GetItemByName('security_card_01') then return end
    TriggerClientEvent('ta2-bankrobbery:UseBankcardA', source)
end)

TA2Core.Functions.CreateUseableItem('security_card_02', function(source)
    local Player = TA2Core.Functions.GetPlayer(source)
    if not Player or not Player.Functions.GetItemByName('security_card_02') then return end
    TriggerClientEvent('ta2-bankrobbery:UseBankcardB', source)
end)

TA2Core.Functions.CreateUseableItem('electronickit', function(source)
    local Player = TA2Core.Functions.GetPlayer(source)
