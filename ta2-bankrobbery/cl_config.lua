...
function Config.DoorlockAction(doorId, setLocked)
    TriggerServerEvent('ta2-doorlock:server:updateState', doorId, setLocked, false, false, true, false, false)
end
...
function Config.OnEvidence(pos, chance)
    if math.random(1, 100) > chance or TA2Core.Functions.IsWearingGloves() then return end
...
function Config.ShowRequiredItems(items, show)
    TriggerEvent('ta2-inventory:client:requiredItems', items, show)
end
