...
RegisterNetEvent('ta2-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('ta2-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('ta2-admin:server:SendReport', name, src, msg)
end)

local function getVehicleFromVehList(hash)
    for _, v in pairs(TA2Core.Shared.Vehicles) do
        if hash == v.hash then
            return v.model
        end
    end
...
RegisterNetEvent('ta2-admin:client:SaveCar', function()
    TA2Core.Functions.TriggerCallback('ta2-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
        ...
        if veh ~= nil and veh ~= 0 then
            ...
            local props = TA2Core.Functions.GetVehicleProperties(veh)
            local hash = props.model
            local vehname = getVehicleFromVehList(hash)
            if TA2Core.Shared.Vehicles[vehname] ~= nil and next(TA2Core.Shared.Vehicles[vehname]) ~= nil then
                TriggerServerEvent('ta2-admin:server:SaveCar', props, TA2Core.Shared.Vehicles[vehname], GetHashKey(veh), plate)
            else
                TA2Core.Functions.Notify(Lang:t('error.no_store_vehicle_garage'), 'error')
            end
        else
...
RegisterNetEvent('ta2-admin:client:SetModel', function(skin)
    TA2Core.Functions.TriggerCallback('ta2-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
        local ped = PlayerPedId()
...
RegisterNetEvent('ta2-admin:client:SetSpeed', function(speed)
    TA2Core.Functions.TriggerCallback('ta2-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
        local ped = PlayerId()
        if speed == 'fast' then
...
RegisterNetEvent('ta2-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('ta2-admin:client:maxmodVehicle', function()
    TA2Core.Functions.TriggerCallback('ta2-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
