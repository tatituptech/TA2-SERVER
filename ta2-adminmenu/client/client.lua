...
RegisterNetEvent('ta2-admin:client:openMenu', function()
    TA2Core.Functions.TriggerCallback('ta2-admin:isAdmin', function(isAdmin)
...
menu2_admin_display_names:On('change', function()
    TriggerEvent('ta2-admin:client:toggleNames')
end)

-- Toggle player blip display
menu2_admin_display_blips:On('change', function()
    TriggerEvent('ta2-admin:client:toggleBlips')
end)
...
-- Weapons list
for _, v in pairs(TA2Core.Shared.Weapons) do
    menu15:AddButton({
        icon = '🎁',
        label = v.label,
        value = v.value,
        description = Lang:t('desc.spawn_weapons_desc'),
        select = function(_)
            TriggerServerEvent('ta2-admin:giveWeapon', v.name)
            TA2Core.Functions.Notify(Lang:t('success.spawn_weapon'))
        end
    })
end
...
local function OpenPermsMenu(permsply)
    TA2Core.Functions.TriggerCallback('ta2-admin:server:getrank', function(rank)
        if rank then
            ...
            menu11:AddButton({
                ...
                select = function(_)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('ta2-admin:server:setPermissions', permsply.id, selectedgroup)
                        TA2Core.Functions.Notify(Lang:t('success.changed_perm'), 'success')
                        selectedgroup = 'Unknown'
                    else
                        TA2Core.Functions.Notify(Lang:t('error.changed_perm_failed'), 'error')
                    end
                end
            })
        else
    ...
    menu10:AddButton({
        ...
        select = function(_)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('ta2-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                TA2Core.Functions.Notify(Lang:t('error.missing_reason'), 'error')
            end
        end
    })
end
    ...
    menu9:AddButton({
        ...
        select = function(_)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('ta2-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                TA2Core.Functions.Notify(Lang:t('error.invalid_reason_length_ban'), 'error')
            end
        end
    })
end
        ...
        Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= 'ban' and values ~= 'kick' and values ~= 'perms' then
                    TriggerServerEvent('ta2-admin:server:' .. values, player)
...
player_management:On('select', function(_)
    menu4:ClearItems()
    TA2Core.Functions.TriggerCallback('test:getplayers', function(players)
    ...
    for _, v in ipairs(elements) do
        menu8:AddButton({
            ...
            select = function(btn)
                ...
                TriggerServerEvent('ta2-weathersync:server:setWeather', selection.value)
                TA2Core.Functions.Notify(Lang:t('weather.weather_changed', { value = selection.label }))
            end
        })
    end
end)
...
menu3_server_time:On('select', function(_, value)
    TriggerServerEvent('ta2-weathersync:server:setTime', value, value)
    TA2Core.Functions.Notify(Lang:t('time.changed', { time = value }))
end)
...
local function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        menu13:AddButton({
            ...
            select = function(_)
                TriggerServerEvent('TA2Core:CallCommand', 'car', { k })
            end
        })
    end
end
...
menu5_vehicles_spawn:On('Select', function(_)
    ...
    for k, v in pairs(vehicles) do
        menu12:AddButton({
            label = TA2Core.Shared.FirstToUpper(k),
            value = v,
...
menu5_vehicles_fix:On('Select', function(_)
    TriggerServerEvent('TA2Core:CallCommand', 'fix', {})
end)

menu5_vehicles_buy:On('Select', function(_)
    TriggerServerEvent('TA2Core:CallCommand', 'admincar', {})
end)

menu5_vehicles_remove:On('Select', function(_)
    TriggerServerEvent('TA2Core:CallCommand', 'dv', {})
end)

menu5_vehicles_max_upgrades:On('Select', function(_)
    TriggerServerEvent('TA2Core:CallCommand', 'maxmods', {})
end)
...
menu7_dev_info_mode:On('change', function(_, _, _)
    ...
    TriggerEvent('ta2-admin:client:ToggleDevmode')
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords2' then
        ...
        local x = TA2Core.Shared.Round(coords.x, 2)
        local y = TA2Core.Shared.Round(coords.y, 2)
        SendNUIMessage({
            string = string.format('vector2(%s, %s)', x, y)
        })
        TA2Core.Functions.Notify(Lang:t('success.coords_copied'), 'success')
    elseif dataType == 'coords3' then
        local coords = GetEntityCoords(ped)
        local x = TA2Core.Shared.Round(coords.x, 2)
        local y = TA2Core.Shared.Round(coords.y, 2)
        local z = TA2Core.Shared.Round(coords.z, 2)
        SendNUIMessage({
    ...
    elseif dataType == 'coords4' then
        ...
        local h = TA2Core.Shared.Round(heading, 2)
        SendNUIMessage({
    ...
    elseif dataType == 'heading' then
        ...
        TA2Core.Functions.Notify(Lang:t('success.heading_copied'), 'success')
    elseif dataType == 'freeaimEntity' then
        ...
        if entity then
            ...
            local x = TA2Core.Shared.Round(entityCoords.x, 2)
            local y = TA2Core.Shared.Round(entityCoords.y, 2)
            local z = TA2Core.Shared.Round(entityCoords.z, 2)
            local rotX = TA2Core.Shared.Round(entityRotation.x, 2)
            local rotY = TA2Core.Shared.Round(entityRotation.y, 2)
            local rotZ = TA2Core.Shared.Round(entityRotation.z, 2)
            local h = TA2Core.Shared.Round(entityHeading, 2)
            SendNUIMessage({
                string = string.format('Model Name:\t%s\nModel Hash:\t%s\n\nHeading:\t%s\nCoords:\t\tvector3(%s, %s, %s)\nRotation:\tvector3(%s, %s, %s)', entityName, entityHash, h, x, y, z, rotX, rotY, rotZ)
            })
            TA2Core.Functions.Notify(Lang:t('success.entity_copy'), 'success')
        else
            TA2Core.Functions.Notify(Lang:t('error.failed_entity_copy'), 'error')
        end
    end
end

RegisterNetEvent('ta2-admin:client:copyToClipboard', function(dataType)
...
local function ToggleShowCoordinates()
    ...
    CreateThread(function()
        while showCoords do
            ...
            local c = {}
            c.x = TA2Core.Shared.Round(coords.x, 2)
            c.y = TA2Core.Shared.Round(coords.y, 2)
            c.z = TA2Core.Shared.Round(coords.z, 2)
            heading = TA2Core.Shared.Round(heading, 2)
            Wait(0)
    ...
    CreateThread(function()
        while vehicleDevMode do
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                Draw2DText(string.format(Lang:t('info.eng_health') .. '~b~%s~s~ | ' .. Lang:t('info.body_health') .. '~b~%s~s~', TA2Core.Shared.Round(eHealth, 2), TA2Core.Shared.Round(bHealth, 2)), 4, { 255, 255, 255 }, 0.4, x + 0.0, y + 0.075)
            end
        end
    end)
end

RegisterNetEvent('ta2-admin:client:ToggleCoords', function()
...
entity_view_distance:On('select', function(_, value)
    ...
    TA2Core.Functions.Notify(Lang:t('info.entity_view_distance', { distance = value }))
end)
    ...
    for _, v in ipairs(elements) do
        EditDealer:AddButton({
            ...
            select = function(btn)
                local values = btn.Value
                if values == 'goto' then
                    TriggerServerEvent('TA2Core:CallCommand', 'dealergoto', { dealer['name'] })
                elseif values == 'remove' then
                    TriggerServerEvent('TA2Core:CallCommand', 'deletedealer', { dealer['name'] })
                    EditDealer:Close()
                    menu6:Close()
                end
            end
...
menu1_dealer_list:On('Select', function(_)
    menu6:ClearItems()
    TA2Core.Functions.TriggerCallback('test:getdealers', function(dealers)
