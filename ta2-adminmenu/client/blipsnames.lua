...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('ta2-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('ta2-admin:client:toggleBlips', function()
    TA2Core.Functions.TriggerCallback('ta2-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
        if not ShowBlips then
            ShowBlips = true
            NetCheck1 = true
            TA2Core.Functions.Notify(Lang:t('success.blips_activated'), 'success')
        else
            ShowBlips = false
            TA2Core.Functions.Notify(Lang:t('error.blips_deactivated'), 'error')
        end
    end)
end)

RegisterNetEvent('ta2-admin:client:toggleNames', function()
    TA2Core.Functions.TriggerCallback('ta2-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
        if not ShowNames then
            ShowNames = true
            NetCheck2 = true
            TA2Core.Functions.Notify(Lang:t('success.names_activated'), 'success')
        else
            ShowNames = false
            TA2Core.Functions.Notify(Lang:t('error.names_deactivated'), 'error')
        end
    end)
end)

RegisterNetEvent('ta2-admin:client:Show', function(players)
    TA2Core.Functions.TriggerCallback('ta2-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
