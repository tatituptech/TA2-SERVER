...
local function DeliverAnim()
    ...
    if Config.UseTarget and hasBag then
        ...
        exports['ta2-target']:RemoveTargetEntity(garbageVehicle)
        if (amountOfBags - 1) <= 0 then
            TA2Core.Functions.TriggerCallback('ta2-garbagejob:server:NextStop', function(hasMoreStops, nextStop, newBagAmount)
                if hasMoreStops and nextStop ~= 0 then
                    ...
                    SetGarbageRoute()
                    TA2Core.Functions.Notify(Lang:t("info.all_bags"))
                    SetVehicleDoorShut(garbageVehicle, 5, false)
                else
                    if hasMoreStops and nextStop == currentStop then
                        TA2Core.Functions.Notify(Lang:t("info.depot_issue"))
                        amountOfBags = 0
                    else
                        -- You are done with work here.
                        TA2Core.Functions.Notify(Lang:t("info.done_working"))
                        SetVehicleDoorShut(garbageVehicle, 5, false)
                        RemoveBlip(deliveryBlip)
                        SetRouteBack()
                        amountOfBags = 0
            ...
            if amountOfBags > 1 then
                TA2Core.Functions.Notify(Lang:t("info.bags_left", { value = amountOfBags }))
            else
                TA2Core.Functions.Notify(Lang:t("info.bags_still", { value = amountOfBags }))
            end
            exports['ta2-target']:AddCircleZone('garbagebin', vector3(CL.coords.x, CL.coords.y, CL.coords.z), 2.0,{
...
function TakeAnim()
    local ped = PlayerPedId()
    TA2Core.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(3000, 5000), false, true, {
        ...
        if Config.UseTarget and not hasBag then
            hasBag = true
            exports['ta2-target']:RemoveZone("garbagebin")
            exports['ta2-target']:AddTargetEntity(garbageVehicle, {
            options = {
        ...
        StopAnimTask(PlayerPedId(), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        TA2Core.Functions.Notify(Lang:t("error.cancled"), "error")
    end)
end
                ...
                if not hasBag and canTakeBag then
                    if Distance < 1.5 then
                        if not GarbText then
                            GarbText = true
                            exports['ta2-core']:DrawText(Lang:t("info.grab_garbage"), 'left')
                        end
                    ...
                    if DoesEntityExist(garbageVehicle) then
                        ...
                        if TruckDist < 2 then
                            if not TrucText then
                                TrucText = true
                                exports['ta2-core']:DrawText(Lang:t("info.dispose_garbage"), 'left')
                            end
                            if IsControlJustPressed(0, 51) and hasBag then
                                StopAnimTask(PlayerPedId(), 'missfbi4prepp1', '_bag_walk_garbage_man', 1.0)
                                DeliverAnim()
                                TA2Core.Functions.Progressbar("deliverbag", Lang:t("info.progressbar"), 2000, false, true, {
                                        disableMovement = true,
                                        disableCarMovement = true,
                                        disableMouse = false,
                                        disableCombat = true,
                        ...
                        TA2Core.Functions.Notify(Lang:t("error.no_truck"), "error")
                        hasBag = false
                    end
                end
            end
    ...
    CreateThread(function()
        while ControlListen do
            if IsControlJustReleased(0, 38) then
                TriggerEvent("ta2-garbagejob:client:MainMenu")
            end
            Wait(1)
        end
    end)
...
local function spawnPeds()
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['ta2-target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "ta2-garbagejob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-recycle', job = "garbage",}},
                distance = 2.0
            })
        else
            ...
            if options then
                ...
                zone:onPlayerInOut(function(inside)
                    if LocalPlayer.state.isLoggedIn then
                        if inside then
                            exports['ta2-core']:DrawText(Lang:t("info.talk"), 'left')
                            Listen4Control()
                        else
                            ControlListen = false
                            exports['ta2-core']:HideText()
...
RegisterNetEvent('ta2-garbagejob:client:SetWaypointHome', function()
    SetNewWaypoint(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y)
end)

RegisterNetEvent('ta2-garbagejob:client:RequestRoute', function()
    if garbageVehicle then continueworking = true TriggerServerEvent('ta2-garbagejob:server:PayShift', continueworking) end
    TA2Core.Functions.TriggerCallback('ta2-garbagejob:server:NewShift', function(shouldContinue, firstStop, totalBags)
        if shouldContinue then
            if not garbageVehicle then
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 2.5) then
                        TA2Core.Functions.TriggerCallback('TA2Core:Server:SpawnVehicle', function(netId)
                            ...
                            garbageVehicle = veh
                            SetVehicleNumberPlateText(veh, "ta2-" .. tostring(math.random(1000, 9999)))
                            ...
                            TA2Core.Functions.Notify(Lang:t("info.deposit_paid", { value = Config.TruckPrice }))
                            TA2Core.Functions.Notify(Lang:t("info.started"))
                            TriggerServerEvent("ta2-garbagejob:server:payDeposit")
                ...
                if occupied then
                    TA2Core.Functions.Notify(Lang:t("error.all_occupied"))
                end
            end
            currentStop = firstStop
            currentStopNum = 1
            ...
            TA2Core.Functions.Notify(Lang:t("info.not_enough", { value = Config.TruckPrice }))
        end
    end, continueworking)
end)

RegisterNetEvent('ta2-garbagejob:client:RequestPaycheck', function()
    if garbageVehicle then
        BringBackCar()
        TA2Core.Functions.Notify(Lang:t("info.truck_returned"))
    end
    TriggerServerEvent('ta2-garbagejob:server:PayShift')
end)

RegisterNetEvent('ta2-garbagejob:client:MainMenu', function()
    if playerJob.name == Config.Jobname then
        local MainMenu = {}
        MainMenu[#MainMenu+1] = {isMenuHeader = true,header = Lang:t("menu.header")}
        MainMenu[#MainMenu+1] = { header = Lang:t("menu.collect"),txt = Lang:t("menu.return_collect"),params = { event = 'ta2-garbagejob:client:RequestPaycheck',}}
        if not garbageVehicle or finished then
            MainMenu[#MainMenu+1] = { header = Lang:t("menu.route"), txt = Lang:t("menu.request_route"), params = { event = 'ta2-garbagejob:client:RequestRoute',}}
        end
        exports['ta2-menu']:openMenu(MainMenu)
    else
        TA2Core.Functions.Notify(Lang:t("error.job"))
    end
end)
