    ...
    CreateThread(function()
        while fireworkTime > 0 do
            if #(pedCoords - vector3(coords.x, coords.y, coords.z)) < 50.0 then
               exports['ta2-core']:DrawText(Lang:t('firework.time_left') .. ' ' .. fireworkTime)
            end
            Wait(1000)
            fireworkTime -= 1
        end
...
RegisterNetEvent('fireworks:client:UseFirework', function(itemName, assetName)
    TA2Core.Functions.Progressbar('spawn_object', Lang:t('firework.place_progress'), 3000, false, true, {
        ...
        StopAnimTask(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 1.0)
        TA2Core.Functions.Notify(Lang:t('firework.canceled'), 'error')
    end)
end)
