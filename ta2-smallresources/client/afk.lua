...
local function updatePermissionLevel()
    TA2Core.Functions.TriggerCallback('ta2-afkkick:server:GetPermissions', function(userGroups)
...
RegisterNetEvent('TA2Core:Client:OnPermissionUpdate', function()
    updatePermissionLevel()
end)
                        ...
                        if time then
                            if time > 0 then
                                ...
                                if _type == 'minutes' then
                                    TA2Core.Functions.Notify(Lang:t('afk.will_kick') .. math.ceil(time / 60) .. Lang:t('afk.time_minutes'), 'error', 10000)
                                elseif _type == 'seconds' then
                                    TA2Core.Functions.Notify(Lang:t('afk.will_kick') .. time .. Lang:t('afk.time_seconds'), 'error', 10000)
                                end
                                time -= 10
                            else
