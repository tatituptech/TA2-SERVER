...
local function triggerCruiseControl(veh)
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if speed > 0 and GetVehicleCurrentGear(veh) > 0 then
            ...
            if GetResourceState('ta2-fuel') == 'started' then
                if exports['ta2-fuel']:GetFuel(veh) <= 10 then
                    TA2Core.Functions.Notify(Lang:t('cruise.not_Enough_Fuel'), 'error')
                    return
                end
            end
            ...
            TA2Core.Functions.Notify(Lang:t('cruise.activated'))
            CreateThread(function()
                while speed > 0 and GetPedInVehicleSeat(veh, -1) == ped do
                    ...
                    if not isTurningOrHandbraking and GetEntitySpeed(veh) < speed - 1.5 then
                        ...
                        TA2Core.Functions.Notify(Lang:t('cruise.deactivated'), 'error')
                        Wait(2000)
                        break
                    end
