...
local function findVehFromPlateAndLocate(plate)
    local gameVehicles = TA2Core.Functions.GetVehicles()
    for i = 1, #gameVehicles do
        local vehicle = gameVehicles[i]
        if DoesEntityExist(vehicle) then
            if TA2Core.Functions.GetPlate(vehicle) == plate then
...
local function LoadPhone()
    Wait(100)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetPhoneData', function(pData)
        PlayerJob = TA2Core.Functions.GetPlayerData().job
        PhoneData.PlayerData = TA2Core.Functions.GetPlayerData()
...
local function OpenPhone()
    TA2Core.Functions.TriggerCallback('ta2-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            PhoneData.PlayerData = TA2Core.Functions.GetPlayerData()
            ...
            TA2Core.Functions.TriggerCallback('ta2-garages:server:GetPlayerVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            TA2Core.Functions.Notify("You don't have a phone", 'error')
        end
    end)
end
...
local function CancelCall()
    TriggerServerEvent('ta2-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('ta2-phone:server:SetCallState', false)
...
local function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('ta2-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('ta2-phone:server:SetCallState', true)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('ta2-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            TA2Core.Functions.Notify('Action not available at the moment..', 'error')
        end
    end
end)
...
RegisterNUICallback('ClearRecentAlerts', function(_, cb)
    TriggerServerEvent('ta2-phone:server:SetPhoneAlerts', 'phone', 0)
...
RegisterNUICallback('SetBackground', function(data, cb)
    ...
    TriggerServerEvent('ta2-phone:server:SaveMetaData', PhoneData.MetaData)
    cb('ok')
end)
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('ta2-phone:server:RemoveMail', MailId)
    cb('ok')
end)
...
RegisterNUICallback('AcceptMailButton', function(data, cb)
    ...
    TriggerServerEvent('ta2-phone:server:ClearButtonData', data.mailId)
    cb('ok')
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('ta2-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('GetInvoices', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetInvoices', function(resp)
        cb(resp)
    end)
end)
...
RegisterNUICallback('PostAdvert', function(data, cb)
    TriggerServerEvent('ta2-phone:server:AddAdvert', data.message, data.url)
    cb('ok')
end)

RegisterNUICallback('DeleteAdvert', function(_, cb)
    TriggerServerEvent('ta2-phone:server:DeleteAdvert')
    cb('ok')
end)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('ta2-phone:server:SetPhoneAlerts', 'whatsapp', newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    TA2Core.Functions.TriggerCallback('ta2-phone:server:PayInvoice', function(resp)
        cb(resp)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('ta2-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    local amount = data.amount
    local invoiceId = data.invoiceId
    TA2Core.Functions.TriggerCallback('ta2-phone:server:DeclineInvoice', function(resp)
        cb(resp)
    end, society, amount, invoiceId)
    TriggerServerEvent('ta2-phone:server:BillingEmail', data, false)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('ta2-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
    ...
    if #Hashtag <= 3 then
        for i = 2, #Hashtag, 1 do
            ...
            if Handle ~= nil or Handle ~= '' then
                ...
                TriggerServerEvent('ta2-phone:server:UpdateHashtags', Handle, TweetMessage)
            end
        end
            ...
            if Handle ~= nil or Handle ~= '' then
                ...
                if (Firstname ~= nil and Firstname ~= '') and (Lastname ~= nil and Lastname ~= '') then
                    if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                        TriggerServerEvent('ta2-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                    end
                end
            end
        end
        ...
        TriggerServerEvent('ta2-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
    else
        SendNUIMessage({
            action = 'PhoneNotification',
            PhoneNotify = {
...
RegisterNUICallback('DeleteTweet', function(data, cb)
    TriggerServerEvent('ta2-phone:server:DeleteTweet', data.id)
    cb('ok')
end)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('ta2-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('ta2-phone:server:RemoveInstallation', data.app)
    cb('ok')
end)

RegisterNUICallback('GetTruckerData', function(_, cb)
    local TruckerMeta = TA2Core.Functions.GetPlayerData().metadata['jobrep']['trucker']
    local TierData = exports['ta2-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
...
RegisterNUICallback('DeleteImage', function(image, cb)
    TriggerServerEvent('ta2-phone:server:RemoveImageFromGallery', image)
    Wait(400)
    TriggerServerEvent('ta2-phone:server:getImageFromGallery')
    cb(true)
end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        TA2Core.Functions.Notify('Your vehicle has been marked', 'success')
    else
        TA2Core.Functions.Notify('This vehicle cannot be located', 'error')
    end
    cb('ok')
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('ta2-phone:server:RemoveContact', Name, Number)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data, cb)
    TriggerServerEvent('ta2-lapraces:server:JoinRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('LeaveRace', function(data, cb)
    TriggerServerEvent('ta2-lapraces:server:LeaveRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('StartRace', function(data, cb)
    TriggerServerEvent('ta2-lapraces:server:StartRace', data.RaceData.RaceId)
    cb('ok')
end)

RegisterNUICallback('SetAlertWaypoint', function(data, cb)
    local coords = data.alert.coords
    TA2Core.Functions.Notify('GPS Location set: ' .. data.alert.title)
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
            for k, _ in pairs(result) do
                TA2Core.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(_, cb)
    ...
    TA2Core.Functions.TriggerCallback('ta2-phone:server:ScanPlate', function(result)
        TA2Core.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            if TA2Core.Shared.Vehicles[vehname] ~= nil then
                result.label = TA2Core.Shared.Vehicles[vehname]['name']
            else
...
RegisterNUICallback('GetRaces', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('ta2-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
    cb('ok')
end)

RegisterNUICallback('HasCreatedRace', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(_, cb)
    local InRace = exports['ta2-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['ta2-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('ta2-lapraces:server:CreateLapRace', data.TrackName)
    cb('ok')
end)

RegisterNUICallback('GetRacingLeaderboards', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('ta2-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            TA2Core.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == 'editor' then
        cb(exports['ta2-lapraces']:IsInEditor())
    else
        cb(exports['ta2-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    TA2Core.Functions.Notify('GPS has been set to ' .. data.HouseData.HouseData.adress .. '!', 'success')
    cb('ok')
end)

RegisterNUICallback('RemoveKeyholder', function(data, cb)
    TriggerServerEvent('ta2-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    TA2Core.Functions.TriggerCallback('ta2-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    TA2Core.Functions.Notify('GPS has been set!', 'success')
    cb('ok')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('ClearMentions', function(_, cb)
    ...
    TriggerServerEvent('ta2-phone:server:SetPhoneAlerts', 'twitter', 0)
    SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
    cb('ok')
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data, cb)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('ta2-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
        cb('ok')
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('ta2-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        TA2Core.Functions.TriggerCallback('ta2-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('GetWhatsappChats', function(_, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetContactPictures', function(Chats)
        cb(Chats)
    end, PhoneData.Chats)
end)

RegisterNUICallback('CallContact', function(data, cb)
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetCallState', function(CanCall, IsOnline, _)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('ta2-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('ta2-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    TA2Core.Functions.TriggerCallback('ta2-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = 'UpdateChat',
            chatData = Chat,
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            if Config.Fivemerr == true then
                ...
                return TA2Core.Functions.TriggerCallback('ta2-phone:server:UploadToFivemerr', function(fivemerrData)
                    ...
                    DestroyMobilePhone()
                    CellCamActivate(false, false)
                    TriggerServerEvent('ta2-phone:server:addImageToGallery', imageData.url)
                    Wait(400)
                    TriggerServerEvent('ta2-phone:server:getImageFromGallery')
            ...
            TA2Core.Functions.TriggerCallback('ta2-phone:server:GetWebhook', function(hook)
                if not hook then
                    TA2Core.Functions.Notify('Camera not setup', 'error')
                    return
                end
                ...
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), 'files[]', function(data)
                    ...
                    DestroyMobilePhone()
                    CellCamActivate(false, false)
                    TriggerServerEvent('ta2-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                    Wait(400)
                    TriggerServerEvent('ta2-phone:server:getImageFromGallery')
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        TA2Core.Functions.Notify('You need to input a Player ID', 'error')
    else
        TriggerServerEvent('ta2-phone:server:sendPing', args[1])
    end
end, false)
...
RegisterNetEvent('ta2-phone:client:TransferMoney', function(amount, newmoney)
...
-- RegisterNetEvent('ta2-phone:client:UpdateTweetsDel', function(source, Tweets)
...
RegisterNetEvent('ta2-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
    PhoneData.Tweets = Tweets
    local MyPlayerId = PhoneData.PlayerData.source
...
RegisterNetEvent('ta2-phone:client:RaceNotify', function(message)
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = 'Racing',
...
RegisterNetEvent('ta2-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('ta2-phone:server:SetPhoneAlerts', 'phone')
...
RegisterNetEvent('ta2-phone-new:client:BankNotify', function(text)
    SendNUIMessage({
        action = 'PhoneNotification',
        NotifyData = {
            title = 'Bank',
...
RegisterNetEvent('ta2-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = 'Mail',
    ...
    TriggerServerEvent('ta2-phone:server:SetPhoneAlerts', 'mail')
end)

RegisterNetEvent('ta2-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = 'UpdateMails',
        Mails = NewMails
    })
...
RegisterNetEvent('ta2-phone:client:UpdateAdvertsDel', function(Adverts)
...
RegisterNetEvent('ta2-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
...
RegisterNetEvent('ta2-phone:client:BillingEmail', function(data, paid, name)
    if paid then
...
RegisterNetEvent('ta2-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == 'ongoing' then
        SendNUIMessage({
...
RegisterNetEvent('ta2-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('ta2-phone:server:AddRecentCall', 'missed', CallData)
                    break
                end
                Wait(Config.RepeatTimeout)
            else
...
RegisterNetEvent('ta2-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            TriggerServerEvent('ta2-phone:server:SetPhoneAlerts', 'whatsapp')
        end
    else
...
RegisterNetEvent('ta2-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = 'PhoneNotification',
            PhoneNotify = {
...
RegisterNetEvent('ta2-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNetEvent('ta2-phone:client:AddTransaction', function(_, _, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('ta2-phone:server:AddTransaction', Data)
end)

RegisterNetEvent('ta2-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications['phone'].Alerts = Config.PhoneApplications['phone'].Alerts + 1
    TriggerServerEvent('ta2-phone:server:SetPhoneAlerts', 'phone', Config.PhoneApplications['phone'].Alerts)
end)

RegisterNetEvent('ta2-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNetEvent('ta2-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = 'ongoing'
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNetEvent('ta2-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = TA2Core.Functions.GetPlayerData().job
...
RegisterNetEvent('ta2-phone:client:GiveContactDetails', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('ta2-phone:server:GiveContactDetails', PlayerId)
    else
        TA2Core.Functions.Notify('No one nearby!', 'error')
    end
end)

RegisterNetEvent('ta2-phone:client:UpdateLapraces', function()
    SendNUIMessage({
        action = 'UpdateRacingApp',
    })
end)

RegisterNetEvent('ta2-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNetEvent('ta2-phone:refreshImages', function(images)
    PhoneData.Images = images
end)

RegisterNetEvent('ta2-phone:client:CustomNotification', function(title, text, icon, color, timeout) -- Send a PhoneNotification to the phone from anywhere
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = title,
