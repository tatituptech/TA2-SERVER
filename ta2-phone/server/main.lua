...
local function GetOnlineStatus(number)
    local Target = TA2Core.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then
        retval = true
    end
...
local function sendNewMailToOffline(citizenid, mailData)
    ...
    if Player then
        ...
        if mailData.button == nil then
            ...
            TriggerClientEvent('ta2-phone:client:NewMailNotify', src, mailData)
        else
        ...
        SetTimeout(200, function()
            ...
            TriggerClientEvent('ta2-phone:client:UpdateMails', src, mails)
        end)
    else
        if mailData.button == nil then
...
TA2Core.Functions.CreateCallback("ta2-phone:server:GetInvoices", function(source, cb)
    ...
    if Player then
        ...
        for _, v in pairs(invoices) do
            local Ply = TA2Core.Functions.GetPlayerByCitizenId(v.sender)
            if Ply ~= nil then
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetCallState', function(_, cb, ContactData)
    local Target = TA2Core.Functions.GetPlayerByPhone(ContactData.number)
    if Target ~= nil then
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetPhoneData', function(source, cb)
    local src = source
...
TA2Core.Functions.CreateCallback('ta2-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Ply = TA2Core.Functions.GetPlayer(source)
    local SenderPly = TA2Core.Functions.GetPlayerByCitizenId(sendercitizenid)
    ...
    if Ply then
        ...
        if exists[1] and exists[1]["count"] == 1 then
            if SenderPly and Config.BillingCommissions[society] then
                local commission = TA2Core.Shared.Round(amount * Config.BillingCommissions[society])
            ...
            if Ply.Functions.RemoveMoney('bank', amount, 'paid-invoice') then
                ...
                if invoiceMailData then
                    exports['ta2-phone']:sendNewMailToOffline(sendercitizenid, invoiceMailData)
                end
                TriggerEvent("ta2-phone:server:paidInvoice", source, invoiceId)
                exports['ta2-banking']:AddMoney(society, amount, 'Phone invoice')
                cb(true)
                return
            end
        end
...
TA2Core.Functions.CreateCallback('ta2-phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Ply = TA2Core.Functions.GetPlayer(source)
    if Ply then
        ...
        if exists[1] and exists[1]["count"] == 1 then
            TriggerEvent("ta2-phone:server:declinedInvoice", source, invoiceId)
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetContactPictures', function(_, cb, Chats)
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetContactPicture', function(_, cb, Chat)
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if result[1] ~= nil then
...
TA2Core.Functions.CreateCallback('ta2-phone:server:FetchResult', function(_, cb, search)
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
    ...
    if result[1] ~= nil then
        for k, _ in pairs(result) do
            ...
            if player[1] ~= nil then
                ...
                local vehicleInfo = TA2Core.Shared.Vehicles[result[k].vehicle]
...
TA2Core.Functions.CreateCallback('ta2-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate ~= nil then
        ...
        TriggerClientEvent('TA2Core:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)

TA2Core.Functions.CreateCallback('ta2-phone:server:HasPhone', function(source, cb)
    local Player = TA2Core.Functions.GetPlayer(source)
    if Player ~= nil then
...
TA2Core.Functions.CreateCallback('ta2-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
        if result[1] ~= nil then
            local Reciever = TA2Core.Functions.GetPlayerByCitizenId(result[1].citizenid)
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
    for _, v in pairs(TA2Core.Functions.GetPlayers()) do
        local Player = TA2Core.Functions.GetPlayer(v)
        if Player ~= nil then
...
TA2Core.Functions.CreateCallback('ta2-phone:server:GetWebhook', function(_, cb)
    if WebHook ~= '' then
        cb(WebHook)
    else
...
TA2Core.Functions.CreateCallback('ta2-phone:server:UploadToFivemerr', function(source, cb)
    local src = source
...
RegisterNetEvent('ta2-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ta2-phone:client:UpdateAdverts', -1, Adverts, '@' .. Player.PlayerData.charinfo.firstname .. '' .. Player.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('ta2-phone:server:DeleteAdvert', function()
    local Player = TA2Core.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('ta2-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('ta2-phone:server:SetCallState', function(bool)
    local src = source
    local Ply = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { Player.PlayerData.citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
...
RegisterNetEvent('ta2-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = TA2Core.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('ta2-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('ta2-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for _, v in pairs(TA2Core.Functions.GetPlayers()) do
        local Player = TA2Core.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('ta2-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]['twitter'])
            else
...
RegisterNetEvent('ta2-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = TA2Core.Functions.GetPlayer(src)
    local Target = TA2Core.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
        TriggerClientEvent('ta2-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

RegisterNetEvent('ta2-phone:server:BillingEmail', function(data, paid)
    for _, v in pairs(TA2Core.Functions.GetPlayers()) do
        local target = TA2Core.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. TA2Core.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. TA2Core.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('ta2-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. TA2Core.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. TA2Core.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('ta2-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('ta2-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('ta2-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

RegisterNetEvent('ta2-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = TA2Core.Functions.GetPlayer(src).citizenid
    QBPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterNetEvent('ta2-phone:server:DeleteTweet', function(tweetId)
    local Player = TA2Core.Functions.GetPlayer(source)
    local delete = false
    local TID = tweetId
    ...
    if delete then
        ...
        TriggerClientEvent('ta2-phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)

RegisterNetEvent('ta2-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    ...
    TriggerClientEvent('ta2-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
end)

RegisterNetEvent('ta2-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = TA2Core.Functions.GetPlayer(src)
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('ta2-phone:client:TransferMoney', reciever.PlayerData.source, amount,
                    reciever.PlayerData.money.bank)
            end
        else
            local moneyInfo = json.decode(result[1].money)
            moneyInfo.bank = TA2Core.Shared.Round(moneyInfo.bank + amount)
            MySQL.update('UPDATE players SET money = ? WHERE citizenid = ?',
                { json.encode(moneyInfo), result[1].citizenid })
            sender.Functions.RemoveMoney('bank', amount, 'phone-transfered')
        end
    else
        TriggerClientEvent('TA2Core:Notify', src, "This account number doesn't exist!", 'error')
    end
end)

RegisterNetEvent('ta2-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    MySQL.update(
...
RegisterNetEvent('ta2-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, _)
    local src = source
    local SenderData = TA2Core.Functions.GetPlayer(src)
    local query = '%' .. ChatNumber .. '%'
    local Player = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if Player[1] ~= nil then
        local TargetData = TA2Core.Functions.GetPlayerByCitizenId(Player[1].citizenid)
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('ta2-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
            else
                -- Insert for target
                ...
                -- Send notification & Update messages for target
                TriggerClientEvent('ta2-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?', { SenderData.PlayerData.citizenid, ChatNumber })
            if Chat[1] ~= nil then
...
RegisterNetEvent('ta2-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = TA2Core.Functions.GetPlayer(src)
    local Hour = os.date('%H')
    local Minute = os.date('%M')
    local label = Hour .. ':' .. Minute
    TriggerClientEvent('ta2-phone:client:AddRecentCall', src, data, label, type)
    local Trgt = TA2Core.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('ta2-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterNetEvent('ta2-phone:server:CancelCall', function(ContactData)
    local Ply = TA2Core.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('ta2-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('ta2-phone:server:AnswerCall', function(CallData)
    local Ply = TA2Core.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('ta2-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('ta2-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('ta2-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('ta2-phone:server:AddTransaction', function(data)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('ta2-phone:RefreshPhone', src)
end)

RegisterNetEvent('ta2-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:addImageToGallery', function(image)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    MySQL.insert('INSERT INTO phone_gallery (`citizenid`, `image`) VALUES (?, ?)', { Player.PlayerData.citizenid, image })
end)

RegisterNetEvent('ta2-phone:server:getImageFromGallery', function()
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC', { Player.PlayerData.citizenid })
    TriggerClientEvent('ta2-phone:refreshImages', src, images)
end)

RegisterNetEvent('ta2-phone:server:RemoveImageFromGallery', function(data)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
...
RegisterNetEvent('ta2-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        TriggerClientEvent('TA2Core:Notify', src, 'You cannot ping yourself', 'error')
    end
end)

-- Command

TA2Core.Commands.Add('setmetadata', 'Set Player Metadata (God Only)', {}, false, function(source, args)
    local Player = TA2Core.Functions.GetPlayer(source)
    if args[1] then
...
TA2Core.Commands.Add('bill', 'Bill A Player', { { name = 'id', help = 'Player ID' }, { name = 'amount', help = 'Fine Amount' } }, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('TA2Core:Notify', source, 'Invoice Successfully Sent', 'success')
                    TriggerClientEvent('TA2Core:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('TA2Core:Notify', source, 'Must Be A Valid Amount Above 0', 'error')
                end
            else
                TriggerClientEvent('TA2Core:Notify', source, 'You Cannot Bill Yourself', 'error')
            end
        else
            TriggerClientEvent('TA2Core:Notify', source, 'Player Not Online', 'error')
        end
    else
        TriggerClientEvent('TA2Core:Notify', source, 'No Access', 'error')
    end
end)
