...
local function enterRobberyHouse(house)
    ...
    local data = exports['ta2-interior']:CreateHouseRobbery(coords)
...
local function searchCabin(cabin)
    local ped = PlayerPedId()
    if math.random(1, 100) <= 85 and not TA2Core.Functions.IsWearingGloves() then
    ...
    TriggerServerEvent('ta2-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    FreezeEntityPosition(ped, true)
    IsLockpicking = true
    local success = exports['ta2-minigames']:Skillbar()
    if success then
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('ta2-houserobbery:server:searchFurniture', cabin, currentHouse)
        Config.Houses[currentHouse]['furniture'][cabin]['searched'] = true
        TriggerServerEvent('ta2-houserobbery:server:SetBusyState', cabin, currentHouse, false)
        FreezeEntityPosition(ped, false)
        IsLockpicking = false
    else
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('ta2-houserobbery:server:SetBusyState', cabin, currentHouse, false)
        TA2Core.Functions.Notify(Lang:t('error.process_cancelled'), 'error', 3500)
        FreezeEntityPosition(ped, false)
        IsLockpicking = false
    end
end
...
RegisterNetEvent('TA2Core:Client:OnPlayerLoaded', function()
    TA2Core.Functions.TriggerCallback('ta2-houserobbery:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('ta2-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('ta2-houserobbery:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('ta2-houserobbery:client:setHouseState', function(house, state)
    Config.Houses[house]['opened'] = state
end)

RegisterNetEvent('ta2-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]['furniture'][cabin]['searched'] = state
end)

RegisterNetEvent('ta2-houserobbery:client:SetBusyState', function(cabin, house, bool)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    if Config.UseClockHours then
        if GetClockHours() < Config.MinimumTime or GetClockHours() > Config.MaximumTime then
            TA2Core.Functions.Notify(Lang:t('error.not_allowed_time'), 'error', 3500)
            return
        end
    end
        ...
        if CurrentCops >= Config.PoliceOnDutyRequired then
            if not Config.Houses[closestHouse]['opened'] then
                if not usingAdvanced then
                    if Config.RequireScrewdriver and not TA2Core.Functions.HasItem('screwdriverset') then
                        TA2Core.Functions.Notify(Lang:t('error.missing_something'), 'error', 3500)
                        return
                    end
                end
                ...
                if usingAdvanced then
                    local success = exports['ta2-minigames']:Skillbar()
                    if success then
                        TriggerServerEvent('ta2-houserobbery:server:enterHouse', closestHouse)
                        TA2Core.Functions.Notify(Lang:t('success.worked'), 'success', 2500)
                    else
                        if math.random(1, 100) <= Config.ChanceToBreakAdvancedLockPick then
                            TriggerServerEvent('ta2-houserobbery:server:removeAdvancedLockpick')
                            TriggerEvent('ta2-inventory:client:ItemBox', TA2Core.Shared.Items['advancedlockpick'], 'remove')
                        end
                        TA2Core.Functions.Notify(Lang:t('error.didnt_work'), 'error', 2500)
                    end
                else
                    local success = exports['ta2-minigames']:Skillbar('medium')
                    if success then
                        TriggerServerEvent('ta2-houserobbery:server:enterHouse', closestHouse)
                        TA2Core.Functions.Notify(Lang:t('success.worked'), 'success', 2500)
                    else
                        if math.random(1, 100) <= Config.ChanceToBreakLockPick then
                            TriggerServerEvent('ta2-houserobbery:server:removeLockpick')
                ...
                TA2Core.Functions.Notify(Lang:t('error.door_open'), 'error', 3500)
            end
        else
            TA2Core.Functions.Notify(Lang:t('error.not_enough_police'), 'error', 3500)
        end
    end
end)
...
CreateThread(function()
    Wait(500)
    local requiredItems = {
        [1] = { name = TA2Core.Shared.Items['advancedlockpick']['name'], image = TA2Core.Shared.Items['advancedlockpick']['image'] },
        [2] = { name = TA2Core.Shared.Items['screwdriverset']['name'], image = TA2Core.Shared.Items['screwdriverset']['image'] },
    }
