...
AddEventHandler('TA2Core:Client:OnPlayerLoaded', function()
    ...
    if player.metadata.tracker then
        ...
        TriggerEvent('ta2-clothing:client:loadOutfit', trackerClothingData)
    else
        local trackerClothingData = {
            outfitData = {
                ['accessory'] = {
...
RegisterNetEvent('TA2Core:Client:SetDuty', function(newDuty)
...
RegisterNetEvent('police:client:policeAlert', function(coords, text)
    ...
    TA2Core.Functions.Notify({ text = text, caption = street1name .. ' ' .. street2name }, 'police')
