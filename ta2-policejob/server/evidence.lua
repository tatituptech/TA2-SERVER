...
TA2Core.Functions.CreateCallback('police:GetPlayerStatus', function(_, cb, playerId)
...
RegisterNetEvent('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    local src = source
    if exports['ta2-inventory']:RemoveItem(src, 'empty_evidence_bag', 1, false, 'evidence:server:AddBlooddropToInventory') then
        if exports['ta2-inventory']:AddItem(src, 'filled_evidence_bag', 1, false, bloodInfo, 'evidence:server:AddBlooddropToInventory') then
...
RegisterNetEvent('evidence:server:AddFingerprintToInventory', function(fingerId, fingerInfo)
    local src = source
    if exports['ta2-inventory']:RemoveItem(src, 'empty_evidence_bag', 1, false, 'evidence:server:AddFingerprintToInventory') then
        if exports['ta2-inventory']:AddItem(src, 'filled_evidence_bag', 1, false, fingerInfo, 'evidence:server:AddFingerprintToInventory') then
            TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['filled_evidence_bag'], 'add')
...
RegisterNetEvent('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local weaponInfo = TA2Core.Shared.Weapons[weapon]
...
RegisterNetEvent('evidence:server:AddCasingToInventory', function(casingId, casingInfo)
    local src = source
    if exports['ta2-inventory']:RemoveItem(src, 'empty_evidence_bag', 1, false, 'evidence:server:AddCasingToInventory') then
        if exports['ta2-inventory']:AddItem(src, 'filled_evidence_bag', 1, false, casingInfo, 'evidence:server:AddCasingToInventory') then
            TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['filled_evidence_bag'], 'add')
