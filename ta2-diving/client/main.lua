...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('ta2-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    TA2Core.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('ta2-diving:server:TakeCoral', CurrentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function setDivingLocation(divingLocation)
    if CurrentDivingLocation.area ~= 0 then
        for k in pairs(Config.CoralLocations[CurrentDivingLocation.area].coords.Coral) do
            if Config.UseTarget then
                exports['ta2-target']:RemoveZone(k)
            else
    ...
    for k, v in pairs(Config.CoralLocations[CurrentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            exports['ta2-target']:AddBoxZone('diving_coral_zone_'..k, v.coords, v.length, v.width, {
            ...
            Zones[k]:onPlayerInOut(function(inside)
                if inside then
                    CurrentArea = k
                    exports['ta2-core']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    CurrentArea = 0
...
local function sellCoral()
    ...
    TA2Core.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ClearPedTasks(playerPed)
        TriggerServerEvent('ta2-diving:server:SellCorals')
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['ta2-core']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['ta2-core']:HideText()
                end
...
RegisterNetEvent('TA2Core:Client:OnPlayerLoaded', function()
    TA2Core.Functions.TriggerCallback('ta2-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('ta2-diving:client:NewLocations', function()
    TA2Core.Functions.TriggerCallback('ta2-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('ta2-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('ta2-diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("ta2-diving:client:SetOxygenLevel", function()
    if OxygenLevel == 0 then
       OxygenLevel = Config.OxygenLevel -- oxygenlevel
       TA2Core.Functions.Notify(Lang:t("success.tube_filled"), 'success')
       TriggerServerEvent('ta2-diving:server:removeItemAfterFill')
    else
        TA2Core.Functions.Notify(Lang:t("error.oxygenlevel", {oxygenlevel = OxygenLevel}), 'error')
    end
end)
...
RegisterNetEvent('ta2-diving:client:UseGear', function()
    local ped = PlayerPedId()
    if isWearingSuit == false then
        if OxygenLevel > 0 then
            ...
            if not IsPedSwimming(ped) and not IsPedInAnyVehicle(ped, false) then
                gearAnim()
                TA2Core.Functions.Progressbar("equip_gear", Lang:t("info.put_suit"), 5000, false, true, {}, {}, {}, {},
                    function() -- Done
                        deleteGear()
                ...
                TA2Core.Functions.Notify(Lang:t("error.not_standing_up"), 'error')
            end
        else
            TA2Core.Functions.Notify(Lang:t("error.need_otube"), 'error')
        end
    elseif isWearingSuit == true then
        gearAnim()
        TA2Core.Functions.Progressbar("remove_gear", Lang:t("info.pullout_suit"), 5000, false, true, {}, {}, {}, {}, function() -- Done
            SetEnableScuba(ped, false)
            SetPedMaxTimeUnderwater(ped, 50.00)
            CurrentGear.enabled = false
            ClearPedTasks(ped)
            deleteGear()
            TA2Core.Functions.Notify(Lang:t("success.took_out"))
            TriggerServerEvent("InteractSound_SV:PlayOnSource", nil, 0.25)
            isWearingSuit = false
            OxygenLevel = OxygenLevel
        end)
