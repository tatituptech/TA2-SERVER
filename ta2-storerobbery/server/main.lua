...
RegisterNetEvent('ta2-storerobbery:server:takeMoney', function(register, isDone)
    local src = source
    ...
    if isDone then
        ...
        exports['ta2-inventory']:AddItem(src, 'markedbills', bags, false, info, 'ta2-storerobbery:server:takeMoney')
        ...
        if math.random(1, 100) <= Config.stickyNoteChance then
            ...
            exports['ta2-inventory']:AddItem(src, 'stickynote', 1, false, info, 'ta2-storerobbery:server:takeMoney')
            TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['stickynote'], 'add')
        end
    end
end)

RegisterNetEvent('ta2-storerobbery:server:setRegisterStatus', function(register)
    Config.Registers[register].robbed = true
    Config.Registers[register].time = Config.resetTime
    TriggerClientEvent('ta2-storerobbery:client:setRegisterStatus', -1, register, Config.Registers[register])
end)

RegisterNetEvent('ta2-storerobbery:server:setSafeStatus', function(safe)
    Config.Safes[safe].robbed = true
    TriggerClientEvent('ta2-storerobbery:client:setSafeStatus', -1, safe, true)

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        Config.Safes[safe].robbed = false
        TriggerClientEvent('ta2-storerobbery:client:setSafeStatus', -1, safe, false)
    end)
end)

RegisterNetEvent('ta2-storerobbery:server:SafeReward', function(safe)
    local src = source
    local Player = TA2Core.Functions.GetPlayer(src)
    if not Player then return end
    local playerPed = GetPlayerPed(src)
    ...
    exports['ta2-inventory']:AddItem(src, 'markedbills', bags, false, info, 'ta2-storerobbery:server:SafeReward')
    TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['markedbills'], 'add')
    local luck = math.random(1, 100)
    local odd = math.random(1, 100)
    if luck <= 10 then
        exports['ta2-inventory']:AddItem(src, 'rolex', math.random(3, 7), false, false, 'ta2-storerobbery:server:SafeReward')
        TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['rolex'], 'add')
        if luck == odd then
            Wait(500)
            exports['ta2-inventory']:AddItem(src, 'goldbar', 1, false, false, 'ta2-storerobbery:server:SafeReward')
            TriggerClientEvent('ta2-inventory:client:ItemBox', src, TA2Core.Shared.Items['goldbar'], 'add')
        end
    end
end)

RegisterNetEvent('ta2-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId
    if type == 'safe' then
    ...
    TriggerClientEvent('ta2-storerobbery:client:robberyCall', -1, type, safe, streetLabel, coords)
    TriggerClientEvent('ta2-phone:client:addPoliceAlert', -1, alertData)
end)

RegisterNetEvent('ta2-storerobbery:server:removeAdvancedLockpick', function()
    local Player = TA2Core.Functions.GetPlayer(source)
    if not Player then return end
    exports['ta2-inventory']:RemoveItem(source, 'advancedlockpick', 1, false, 'ta2-storerobbery:server:removeAdvancedLockpick')
end)

RegisterNetEvent('ta2-storerobbery:server:removeLockpick', function()
    local Player = TA2Core.Functions.GetPlayer(source)
    if not Player then return end
    exports['ta2-inventory']:RemoveItem(source, 'lockpick', 1, false, 'ta2-storerobbery:server:removeLockpick')
end)
...
CreateThread(function()
    while true do
        ...
        if #toSend > 0 then
            ...
            TriggerClientEvent('ta2-storerobbery:client:setRegisterStatus', -1, toSend, false)
        end
...
TA2Core.Functions.CreateCallback('ta2-storerobbery:server:isCombinationRight', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

TA2Core.Functions.CreateCallback('ta2-storerobbery:server:getPadlockCombination', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

TA2Core.Functions.CreateCallback('ta2-storerobbery:server:getRegisterStatus', function(_, cb)
    cb(Config.Registers)
end)

TA2Core.Functions.CreateCallback('ta2-storerobbery:server:getSafeStatus', function(_, cb)
    cb(Config.Safes)
end)
